...
WORKFLOWS = {
    ...
    "agency-action": {
        ...
        "title": "Respond to a Government Agency Action",
    },
    "file-appeal": {
        "badge": "Appeal",
        "title_italic": "appeal",
...
def generate_landing_jsx(wf_id, wf):
    key_facts = "\n".join(
        '              <KeyFact label="%s" value="%s" />' % (label, value)
        for label, value in wf["keyfacts"]
    )
    includes_items = "\n".join(
        '                <li className="flex items-center gap-2 text-sm text-ink-soft"><span className="text-stamp">\u25b8</span>%s</li>' % item
        for item in wf["includes"]
    )
    process_steps = "\n".join(
        '              <ProcessStep number="%02d" title="%s" text="%s" />' % (i, t, d)
        for i, (t, d) in enumerate(wf["process"], 1)
    )
    what_paras = "\n".join(
        '              <p>%s</p>' % para
        for para in wf["what_is_paras"]
    )
    related_cards = "\n".join(
        '              <RelatedCard href="%s" title="%s" desc="%s" />' % (href, title, desc)
        for href, title, desc in wf["related"]
    )
        ...
        <section className="relative overflow-hidden border-b border-rule/60">
          ...
          <div className="relative mx-auto max-w-4xl px-4 py-14 sm:px-6 sm:py-20 md:py-28">
            <nav className="flex items-center gap-1.5 text-xs text-muted-foreground" aria-label="Breadcrumb">
              ...
              <span className="text-rule">/</span>
              <span className="text-ink-soft">__BADGE__</span>
            </nav>
            <div className="postmark w-fit mt-6">__BADGE__</div>
            <h1 className="mt-6 font-serif text-4xl leading-[1.1] sm:text-5xl md:text-6xl">
              Respond to your <span className="italic text-stamp">__TITLE_ITALIC__</span>
            </h1>
...
__KEYFACTS__
        ...
        <section className="border-b border-rule/60">
          <div className="mx-auto max-w-3xl px-4 py-12 sm:px-6 sm:py-16">
            ...
            <h2 className="mt-3 font-serif text-3xl leading-tight">__WHAT_IS_TITLE__</h2>
...
__PROCESS__
        ...
        <section ref={workflowRef} className="border-b border-rule/60" style={{ scrollMarginTop: "80px" }}>
          <div className="mx-auto max-w-5xl px-4 py-12 sm:px-6">
            {workflowStarted ? (
""".replace("__BADGE__", wf["badge"]) \
  .replace("__TITLE_ITALIC__", wf["title_italic"]) \
  .replace("__HERO_DESC__", wf["hero_desc"]) \
  .replace("__KEYFACTS__", key_facts) \
  .replace("__WHAT_IS_TITLE__", wf["what_is_title"]) \
  .replace("__WHAT_PARAS__", what_paras) \
  .replace("__INCLUDES__", includes_items) \
  .replace("__PROCESS__", process_steps)

def generate_closing_jsx(wf):
    related_cards = "\n".join(
        '              <RelatedCard href="%s" title="%s" desc="%s" />' % (href, title, desc)
        for href, title, desc in wf["related"]
    )
...
}""".replace("__RELATED__", related_cards)
...
for wf_id, wf in WORKFLOWS.items():
    ...
    # 1. Add imports: useEffect, useRef, Link, SiteHeader, SiteFooter
    ...
    elif 'import { createFileRoute, Link } from "@tanstack/react-router";' in content:
        pass  # already has Link
    elif 'import { createFileRoute } from "@tanstack/react-router";' in content:
        content = content.replace(
            'import { createFileRoute } from "@tanstack/react-router";',
            'import { createFileRoute, Link } from "@tanstack/react-router";'
        )

    # Add useEffect, useRef to React imports
    ...
    # Add SiteHeader and SiteFooter imports
    site_header_import = 'import { SiteHeader } from "@/components/site-header";'
    site_footer_import = 'import { SiteFooter } from "@/components/site-footer";'
    if site_header_import not in content:
        # Add after the workflow-shell import
        if 'from "@/components/workflow-shell"' in content:
            content = content.replace(
                'from "@/components/workflow-shell";',
                'from "@/components/workflow-shell";\n' + site_header_import + '\n' + site_footer_import + ';'
            )
        elif "from '../../domain/workflows';" in content:
            content = content.replace(
                "import { workflows } from '../../domain/workflows';",
                "import { workflows } from '../../domain/workflows';\n" + site_header_import + '\n' + site_footer_import
            )

    # 2. Add workflowStarted state + workflowRef + startWorkflow
    # Find the line with the first useState and add after the last useState in the group
    # Strategy: find the line with "const [done, setDone]" or similar and add after
    # Actually, find a good anchor - after the last state variable declaration before function definitions

    # For the 169-line workflows, there's a pattern of many useState calls
    # For the 145-line ones, similar pattern
    # Let's find "const allChecked" or "const llmAnalysis" or similar

    if "workflowStarted" not in content:
        # Find a good insertion point - after the last state/derived variable, before first function
        # Use the llmAnalysis line or allChecked line
    ...
    # Find "if (done) return <Success" and the "return (" after it
    # Or find "return (" followed by "<WorkflowShell"

    # Find the return with WorkflowShell
    if 'return (\n    <WorkflowShell' in content:
        old_return_start = 'return (\n    <WorkflowShell'
    elif 'return (\n      <WorkflowShell' in content:
        old_return_start = 'return (\n      <WorkflowShell'
    else:
        # Try to find the pattern
        return_match = re.search(r'return \(\s*\n\s*<WorkflowShell', content)
        if return_match:
            old_return_start = return_match.group(0)
        else:
            print("SKIP %s: could not find WorkflowShell return" % wf_id)
            continue

    # Find the end: "  </WorkflowShell>\n  );" or similar
    shell_end_match = re.search(r'</WorkflowShell>\s*\n\s*\);', content)
    if not shell_end_match:
        print("SKIP %s: could not find </WorkflowShell>" % wf_id)
        continue

    shell_end = shell_end_match.group(0)

    # Build the new return
    landing_jsx = generate_landing_jsx(wf_id, wf)
    closing_jsx = generate_closing_jsx(wf)

    # The return block starts at old_return_start and ends at shell_end + possible trailing
    # We need to find the exact span
    start_idx = content.index(old_return_start)
    end_idx = content.index(shell_end) + len(shell_end)

    # Extract the WorkflowShell props to pass through
    shell_block = content[start_idx:end_idx]
    # Extract the <WorkflowShell ...> opening tag props
    shell_open_match = re.search(r'<WorkflowShell\s+([^>]+)>', shell_block)
    shell_props = shell_open_match.group(1) if shell_open_match else 'title="%s"' % wf["title"]

    # Build the new content
    new_return = landing_jsx + '              <WorkflowShell ' + shell_props + '>\n'
    # Extract children (everything between > and </WorkflowShell>)
    children_match = re.search(r'<WorkflowShell[^>]*>\n(.*)</WorkflowShell>', shell_block, re.DOTALL)
    if children_match:
        children = children_match.group(1)
        # Indent children by 14 spaces (they were at 6, need to go deeper)
        # Actually they're already indented, let's just add them
        new_return += children
    new_return += '              </WorkflowShell>\n'
    new_return += '            ' + closing_jsx
