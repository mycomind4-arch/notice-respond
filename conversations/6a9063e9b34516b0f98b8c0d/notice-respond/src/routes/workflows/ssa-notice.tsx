      ...
      {step === 4 && (
        <div>
          ...
          {llmAnalysis.llmAnalysis && (
            <button
              onClick={async () => {
                const r = await fetch("/api/workflows/draft", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ workflowId: "ssa-notice", analysis: llmAnalysis.llmAnalysis, userFacts: facts, userObjective: objective }) });
