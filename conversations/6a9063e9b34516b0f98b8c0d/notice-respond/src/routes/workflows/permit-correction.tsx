...
function permitcorrection() {
  const definition = workflows["permit-correction"];
  ...
  function generateDraft() {
    return `Re: Response to Respond to a Permit Correction Notice
...
[Your Name]`;
  ...
  if (done) return <Success title="Your response has been submitted" href="/workflows/permit-correction" />;
  ...
  return (
    <div className="min-h-screen bg-paper">
      ...
      <main>
        ...
        <section className="relative overflow-hidden border-b border-rule/60">
          ...
          <div className="relative mx-auto max-w-4xl px-4 py-14 sm:px-6 sm:py-20 md:py-28">
            <nav className="flex items-center gap-1.5 text-xs text-muted-foreground" aria-label="Breadcrumb">
              ...
              <span className="text-rule">/</span>
              <span className="text-ink-soft">Permitting Authority</span>
            </nav>
            <div className="postmark w-fit mt-6">Permitting Authority</div>
            <h1 className="mt-6 font-serif text-4xl leading-[1.1] sm:text-5xl md:text-6xl">
              Respond to your <span className="italic text-stamp">permit correction notice</span>
            </h1>
            <p className="mt-6 max-w-2xl text-base leading-7 text-ink-soft sm:text-lg">
              A permitting authority notified you of an issue with a building or construction permit. Upload the notice, understand the required corrections, and prepare a documented response.
            </p>
            ...
            <div className="mt-12 grid grid-cols-2 gap-px overflow-hidden rounded-lg border border-rule/60 bg-rule/60 sm:grid-cols-4">
              <KeyFact label="Notice type" value="Permit Issue" />
        ...
        <section className="border-b border-rule/60">
          <div className="mx-auto max-w-3xl px-4 py-12 sm:px-6 sm:py-16">
            ...
            <h2 className="mt-3 font-serif text-3xl leading-tight">What is a permit correction notice?</h2>
            <div className="mt-6 space-y-4 text-base leading-7 text-ink-soft">
              <p>A permit correction notice is issued by a building department or permitting authority when a construction or land-use permit has errors, missing information, or work that doesn't match the approved plans. It may require corrections, additional documentation, or a re-inspection.</p>
              <p>These notices often come with a deadline and can halt construction until the issue is resolved. Unresolved permit issues can lead to fines, permit revocation, or difficulty selling the property later.</p>
              <p>Responding with the corrected documents, additional information, or a plan for compliance can resolve the issue quickly. You also have the right to contest the notice if you believe the permit was issued correctly.</p>
            </div>
            <div className="mt-8 rounded-lg border border-rule/60 bg-paper-deep/30 p-5">
              ...
              <ul className="mt-3 grid gap-2 sm:grid-cols-2">
                <li className="flex items-center gap-2 text-sm text-ink-soft"><span className="text-stamp">▸</span>Permit number and type</li>
                <li className="flex items-center gap-2 text-sm text-ink-soft"><span className="text-stamp">▸</span>Issuing department</li>
                <li className="flex items-center gap-2 text-sm text-ink-soft"><span className="text-stamp">▸</span>Cited issue or deficiency</li>
                <li className="flex items-center gap-2 text-sm text-ink-soft"><span className="text-stamp">▸</span>Property address</li>
                <li className="flex items-center gap-2 text-sm text-ink-soft"><span className="text-stamp">▸</span>Correction deadline</li>
                <li className="flex items-center gap-2 text-sm text-ink-soft"><span className="text-stamp">▸</span>Required corrective action</li>
                <li className="flex items-center gap-2 text-sm text-ink-soft"><span className="text-stamp">▸</span>Re-inspection requirements</li>
        ...
        <section className="border-b border-rule/60 bg-paper-deep/20">
          <div className="mx-auto max-w-4xl px-4 py-12 sm:px-6 sm:py-16">
            ...
            <div className="mt-8 grid gap-6 md:grid-cols-3">
              <ProcessStep number="01" title="Upload & analyze" text="Upload the permit correction notice. AI extracts the permit number, cited issues, deadline, and required corrections." />
              <ProcessStep number="02" title="Review & draft" text="Document your corrections, provide additional information, or contest the notice. Generate a formal response with supporting evidence." />
              <ProcessStep number="03" title="Mail with proof" text="Approve the exact draft. Certified mail provides proof of timely response before the correction deadline." />
            </div>
          </div>
        </section>
        ...
        <section ref={workflowRef} className="border-b border-rule/60" style={{ scrollMarginTop: "80px" }}>
          <div className="mx-auto max-w-5xl px-4 py-12 sm:px-6">
            {workflowStarted ? (
              <WorkflowShell title="Respond to a Permit Correction Notice" steps={STEPS} step={step} setStep={setStep} canContinue={canContinue()} onNext={next} onBack={() => setStep((s) => Math.max(s - 1, 0))}>
      {step === 0 && (
        <div>
          <div className="postmark w-fit">1 · Start</div>
          <h2 className="mt-4 font-serif text-4xl">Respond to a Permit Correction Notice</h2>
          <p className="mt-3 text-muted-foreground">Turn permit or planning corrections into a tracked response so each requested change is understood, answered, and supported.</p>
          <div className="mt-6 rounded-md border border-rule/70 bg-paper-deep/40 p-4 text-sm text-muted-foreground">
            ...
            <p className="mt-2">{definition?.disclaimer ?? "Permit processes vary by jurisdiction. Notice Respond is not a law firm and does not provide legal advice."}</p>
          </div>
          {(() => { const seo = getWorkflowSEO("permit-correction"); return seo ? <FAQSection faq={seo.faq} /> : null; })()}
        </div>
      )}
      {step === 1 && (
        <div>
          ...
          <div className="mt-6 grid gap-4 sm:grid-cols-2">
            <div><label className="input-label">Agency</label><input className="input-field" value={agency} onChange={(e) => setAgency(e.target.value)} placeholder="Permit Office" /></div>
      ...
      {step === 7 && (<div><div className="postmark w-fit">8 · Recipient</div><h2 className="mt-4 font-serif text-3xl">Where should we send it?</h2><p className="mt-3 text-muted-foreground">Enter the agency's mailing address.</p><RecipientForm recipient={recipient} setRecipient={setRecipient} orgPlaceholder={agency || "Permit Office"} /></div>)}
        ...
        <section className="border-b border-rule/60">
          <div className="mx-auto max-w-3xl px-4 py-12 sm:px-6">
            ...
            <div className="mt-6 grid gap-4 sm:grid-cols-3">
              ...
              <RelatedCard href="/workflows/benefits-notice" title="Benefits Notice" desc="Respond to benefits determination notices" />
            </div>
