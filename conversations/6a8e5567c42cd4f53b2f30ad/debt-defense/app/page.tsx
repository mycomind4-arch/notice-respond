    ...
    <main>
      ...
      <section style={{ position: 'relative', overflow: 'hidden', borderBottom: '1px solid rgba(255,255,255,.08)' }}>
        ...
        <div className="container" style={{ position: 'relative', zIndex: 1, paddingTop: 120, paddingBottom: 100 }}>
          <div className="eyebrow" style={{ color: '#fca5a5' }}>DEBT DEFENSE · {totalWorkflows} WORKFLOWS</div>
          ...
          <p style={{ fontSize: 'clamp(18px,2.5vw,22px)', lineHeight: 1.6, color: '#94a3b8', maxWidth: 740, margin: '0 0 36px' }}>
            Upload the collection letter. We identify the debt, verify the amount, organize your evidence, build a factual dispute or validation request, and deliver with proof of mailing.
          </p>
          <div style={{ display: 'flex', gap: 14, flexWrap: 'wrap' }}>
            <Link href="/workflows" className="btn btn-primary" style={{ background: '#f87171', color: '#061018' }}>Browse all {totalWorkflows} workflows →</Link>
            <a href="#families" className="btn btn-secondary">Browse by category</a>
          </div>
          <div style={{ display: 'flex', gap: 40, marginTop: 56, flexWrap: 'wrap' }}>
            <div><div style={{ fontSize: 36, fontWeight: 800, color: '#f87171' }}>{totalWorkflows}</div><div className="muted" style={{ fontSize: 13 }}>Focused workflows</div></div>
            <div><div style={{ fontSize: 36, fontWeight: 800, color: '#f87171' }}>{families.length}</div><div className="muted" style={{ fontSize: 13 }}>Categories</div></div>
            <div><div style={{ fontSize: 36, fontWeight: 800, color: '#f87171' }}>{(totalSearchVolume / 1000).toFixed(0)}K</div><div className="muted" style={{ fontSize: 13 }}>Monthly search volume</div></div>
            <div><div style={{ fontSize: 36, fontWeight: 800, color: '#f87171' }}>8</div><div className="muted" style={{ fontSize: 13 }}>Pipeline stages</div></div>
          </div>
        </div>
      </section>
