  ...
  return (
    <div ref={containerRef} className="relative w-full">
      ...
      {showResults && results.length > 0 && (
        <div className="absolute top-full mt-2 w-full glass rounded-xl shadow-2xl shadow-black/40 z-50 max-h-80 overflow-y-auto animate-[slide-down_0.2s_ease-out] p-2 space-y-1 [mask-image:linear-gradient(to_bottom,black_85%,transparent)]">
          {results.map((r) => (
            <button
              key={`${r.type}-${r.id}`}
              onClick={() => {
      ...
      {showResults && !loading && results.length === 0 && query.trim().length >= 2 && (
        <div className="absolute top-full mt-2 w-full glass rounded-xl shadow-2xl z-50 p-4 text-center space-y-1 animate-[slide-down_0.2s_ease-out]">
