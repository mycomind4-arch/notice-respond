...
import { analyzeClaim } from "@/products/claim-proof/claude";
...
export const Route = createFileRoute("/api/v1/claim-proof/analyze")({
  server: {
    handlers: {
      POST: async ({ request }) => {
        const limiter = rateLimit(getClientIp(request), "claim-proof.analyze", {
          maxRequests: 10,
          windowMs: 60_000,
        });
        if (!limiter.allowed)
          return errorResponse(429, "rate_limited", "Too many analyses. Please wait a minute and try again.", "CLAIM_ANALYSIS_RATE_LIMITED");

        let input: z.infer<typeof Input>;
        try {
          input = Input.parse(await request.json());
        } catch {
          return errorResponse(400, "invalid_input", "The claim details are incomplete or invalid.", "INVALID_CLAIM_INPUT");
        }

        try {
          const analysis = await analyzeClaim(input);
          return Response.json({ product: "claim-proof", analysis });
        } catch (error) {
          console.error("[claim-proof] analysis failed", error);
          return errorResponse(502, "provider_error", "The claim could not be analyzed. Please try again.", "CLAIM_ANALYSIS_FAILED");
        }
      },
    },
  },
