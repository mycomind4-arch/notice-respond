...
export function MedicaidDenialWorkspace() {
  ...
  async function analyze() {
    ...
    try {
      ...
      const response = await fetch("/api/workflows/medicaid-denial/analyze", { method: "POST", body, credentials: "include", headers: { authorization: `Bearer ${auth}` } });
  ...
  async function build() {
    ...
    try {
      const auth = await token();
      const response = await fetch("/api/workflows/medicaid-denial/draft", { method: "POST", credentials: "include", headers: { "content-type": "application/json", authorization: `Bearer ${auth}` }, body: JSON.stringify({ appealId: result.appealId, analysis: result.analysis }) });
      const payload = await response.json().catch(() => null);
  ...
  async function approve() {
    ...
    try {
      const auth = await token();
      const response = await fetch("/api/workflows/medicaid-denial/approve", { method: "POST", credentials: "include", headers: { "content-type": "application/json", authorization: `Bearer ${auth}` }, body: JSON.stringify({ appealId: result.appealId, recipient, mailingMethod }) });
      const payload = await response.json().catch(() => null);
  ...
  async function checkout() {
    ...
    try {
      const auth = await token();
      const response = await fetch("/api/workflows/medicaid-denial/checkout", { method: "POST", credentials: "include", headers: { "content-type": "application/json", authorization: `Bearer ${auth}` }, body: JSON.stringify({ appealId: result.appealId }) });
      const payload = await response.json().catch(() => null);
  ...
  return <main className="min-h-screen bg-paper px-6 py-10"><div className="mx-auto max-w-5xl">
    <header className="mb-10"><div className="text-xs uppercase tracking-[0.2em] text-muted-foreground">Medicaid denial</div><h1 className="mt-3 font-serif text-4xl md:text-5xl">Understand your denial. Build your appeal. Send it.</h1><p className="mt-4 max-w-2xl text-base leading-7 text-muted-foreground">Upload the Medicaid decision. Gemini analyzes the actual document, builds the response, and keeps you in control before anything is mailed.</p><div className="mt-6 h-1 overflow-hidden rounded-full bg-muted"><div className="h-full bg-foreground transition-all" style={{ width: `${progress}%` }} /></div><div className="mt-2 flex justify-between text-[11px] uppercase tracking-widest text-muted-foreground"><span>Understand</span><span>Build</span><span>Send</span></div></header>
