...
export function InsuranceDenialLetterWorkspace() {
  ...
  async function analyze() {
    ...
    try {
      const token = await authToken(); const form = new FormData(); form.append("document", file);
      const response = await fetch("/api/workflows/insurance-denial-letter/analyze", { method: "POST", body: form, credentials: "include", headers: { authorization: `Bearer ${token}` } });
  ...
  async function build() {
    if (!result?.appealId) return;
    setBusy("draft"); setError(null);
    try {
      const token = await authToken();
      const response = await fetch("/api/workflows/insurance-denial-letter/draft", { method: "POST", credentials: "include", headers: { "content-type": "application/json", authorization: `Bearer ${token}` }, body: JSON.stringify({ appealId: result.appealId, analysis: result.analysis }) });
  ...
  async function approve() {
    if (!result?.appealId || !complete) return;
    setBusy("approve"); setError(null);
    try {
      const token = await authToken();
      const response = await fetch("/api/workflows/insurance-denial-letter/approve", { method: "POST", credentials: "include", headers: { "content-type": "application/json", authorization: `Bearer ${token}` }, body: JSON.stringify({ appealId: result.appealId, recipient, mailingMethod }) });
  ...
  async function checkout() {
    if (!result?.appealId || !approved) return;
    setBusy("checkout"); setError(null);
    try {
      const token = await authToken();
      const response = await fetch("/api/workflows/insurance-denial-letter/checkout", { method: "POST", credentials: "include", headers: { "content-type": "application/json", authorization: `Bearer ${token}` }, body: JSON.stringify({ appealId: result.appealId }) });
  ...
  return <main className="min-h-screen bg-paper px-6 py-10"><div className="mx-auto max-w-5xl">
    <header className="mb-10"><div className="text-xs uppercase tracking-[0.2em] text-muted-foreground">Insurance denial letter</div><h1 className="mt-3 font-serif text-4xl md:text-5xl">Understand it. Build your response. Send it.</h1><p className="mt-4 max-w-2xl text-base leading-7 text-muted-foreground">Upload the denial letter. Gemini reads the actual document, identifies what matters, and helps you prepare the response.</p><div className="mt-6 h-1 overflow-hidden rounded-full bg-muted"><div className="h-full bg-foreground transition-all" style={{ width: `${progress}%` }} /></div><div className="mt-2 flex justify-between text-[11px] uppercase tracking-widest text-muted-foreground"><span>Understand</span><span>Build</span><span>Send</span></div></header>
