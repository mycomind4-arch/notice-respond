...
const categories = [
  { name: "Using Appeal Mail", questions: [
    { q: "Is this legal advice?", a: "No. Appeal Mail is a correspondence tool, not a law firm. We help you prepare and send appeal documents — we do not provide legal advice, and the AI assistant never invents facts or legal conclusions." },
  ...
  { name: "Mailing & Delivery", questions: [
    ...
    { q: "What's proof of timely filing?", a: "Certified mail with return receipt provides a USPS delivery record and a signed card mailed back to you as physical proof." },
  ]},
  ...
  { name: "Legal & Scope", questions: [
    { q: "Is Appeal Mail a law firm?", a: "No. Appeal Mail is a document preparation and mailing service. We do not provide legal advice or representation." },
    { q: "Can this replace an attorney?", a: "No. If your appeal involves complex legal questions, consult a qualified attorney." },
  ]},
];
function FAQPage() {
  return (
    <main className="min-h-screen bg-cream"><SiteHeader />
      <section className="bg-white py-16 md:py-20"><div className="container">
        ...
        <h1 className="mt-3 text-4xl font-bold text-indigo-700 md:text-5xl" style={{ fontFamily: "var(--font-serif)" }}>Frequently asked questions</h1>
        <p className="mt-4 text-slate-400">Everything you need to know about how Appeal Mail works.</p></div>
      </div></section>
      <section className="py-12 md:py-16"><div className="container max-w-3xl">
        {categories.map((cat) => (<div key={cat.name} className="mb-10">
          <h2 className="text-xl font-bold text-indigo-700" style={{ fontFamily: "var(--font-serif)" }}>{cat.name}</h2>
