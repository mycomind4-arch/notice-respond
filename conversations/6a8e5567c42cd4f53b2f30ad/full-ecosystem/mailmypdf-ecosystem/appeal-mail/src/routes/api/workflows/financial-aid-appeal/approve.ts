...
export const Route = createFileRoute("/api/workflows/financial-aid-appeal/approve")({
  server: {
    handlers: {
        POST: async ({ request }) => {
    try {
      ...
      const packet = assemblePacket({ appealId, finalLetter: appeal.draft, evidence, recipient, mailingMethod: input.mailingMethod });
