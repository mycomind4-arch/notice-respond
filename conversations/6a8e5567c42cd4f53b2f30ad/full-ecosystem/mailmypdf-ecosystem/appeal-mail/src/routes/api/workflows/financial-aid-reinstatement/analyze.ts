...
async function resolveGemini() {
  ...
  const r = await fetch(`${base.replace(/\/$/, "")}/api/control-plane/ai`, { method:"POST", headers:{"content-type":"application/json", authorization:`Bearer ${token}`}, body:JSON.stringify({verticalSlug:"appeal-mail", workflowSlug:"financial-aid-reinstatement", task:"analysis"}) });
...
export const Route = createFileRoute("/api/workflows/financial-aid-reinstatement/analyze")({server:{handlers:{POST:async ({request}) => {
  try {
    const user = await requireAuthenticatedUser(request); const workflow = getWorkflow("financial-aid-reinstatement");
    ...
    const prompt = [
      ...
      "Identify prior eligibility status, reason aid was lost or withheld, relevant academic or financial findings, reinstatement requirements, appeal instructions, deadline, evidence referenced, and any recovery-plan requirements.",
  ...
  } catch(error){const message=error instanceof Error?error.message:"Unable to analyze financial-aid decision.";return Response.json({error:message},{status:/authentication|required|token/i.test(message)?401:502});}
}}}});
