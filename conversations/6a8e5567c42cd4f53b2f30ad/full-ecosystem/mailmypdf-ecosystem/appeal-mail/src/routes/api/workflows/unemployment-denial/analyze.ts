...
async function resolveGemini() {
  ...
  const response = await fetch(`${base.replace(/\/$/, "")}/api/control-plane/ai`, {
    ...
    body: JSON.stringify({ verticalSlug: "appeal-mail", workflowSlug: "unemployment-denial", task: "analysis" }),
  });
...
export const Route = createFileRoute("/api/workflows/unemployment-denial/analyze")({
  server: {
    ...
    try {
      ...
      if (!(file instanceof File)) return Response.json({ error: "A source decision is required." }, { status: 400 });
      ...
      const prompt = [
        ...
        "Analyze the actual unemployment decision. Extract only supported facts and clearly separate uncertainty.",
        "Identify eligibility findings, separation/employer facts, claimant statements, agency findings, appeal instructions, deadlines, and supporting records.",
        "Do not invent jurisdiction-specific law or unemployment rules.",
        "Return strict JSON only.",
        '{"summary":"","decision":"","decisionType":"unemployment_denial","issuer":"","referenceNumber":"","decisionDate":"","deadline":"","reasons":[],"keyFacts":[],"issues":[{"issue":"","whyItMatters":"","evidenceNeeded":[]}],"evidenceMentioned":[],"uncertainties":[],"confidence":"high|medium|low"}',
      ].join("\n\n");
      ...
      const analysis = JSON.parse(text) as {
        summary?: string; decision?: string; issuer?: string; referenceNumber?: string; decisionDate?: string; deadline?: string;
      ...
      const decision = createDecision("claim_denial", {
        ...
        decisionTypeLabel: analysis.decision || "Unemployment decision",
        ...
        issues: (analysis.issues || []).map((item, index) => ({ id: `${index}-${crypto.randomUUID()}`, description: item.issue || "Issue identified in unemployment decision", type: "factual_dispute", severity: "medium", sourceExcerpt: item.whyItMatters })),
      ...
      const grounds = (analysis.issues || []).map((issue, index) => createGround("factual_error", {
        ...
        claim: issue.issue || "Review a stated unemployment finding",
      ...
      evidence.unshift(createEvidence("document", "Original unemployment decision", { documentId: document.id, documentFilename: document.filename, uploadedAt: new Date().toISOString() }));
      if (evidence.length && grounds.length) grounds[0].supportingEvidenceIds = evidence.map((item) => item.id);

      const appeal = createAppeal("unemployment-denial", decision);
      ...
      const message = error instanceof Error ? error.message : "Unable to analyze unemployment decision.";
