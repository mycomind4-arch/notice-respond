...
async function resolveGemini() {
  ...
  const response = await fetch(`${base.replace(/\/$/, "")}/api/control-plane/ai`, {
    ...
    body: JSON.stringify({ verticalSlug: "appeal-mail", workflowSlug: "ssdi-denial", task: "analysis" }),
  });
...
export const Route = createFileRoute("/api/workflows/ssdi-denial/analyze")({
  server: {
    handlers: {
      POST: async ({ request }) => {
    try {
      const user = await requireAuthenticatedUser(request);
      const workflow = getWorkflow("ssdi-denial");
      ...
      const prompt = [
        ...
        "Analyze the actual SSDI denial document. Extract only supported facts. Never invent medical facts, diagnoses, limitations, dates, deadlines, or SSA findings.",
        "Identify the decision, denial/reasoning, reconsideration or appeal instructions, deadlines, findings discussed, evidence considered, and evidence gaps.",
        "Return strict JSON only.",
        '{"summary":"","decision":"","decisionType":"ssdi_denial","issuer":"","referenceNumber":"","decisionDate":"","deadline":"","reasons":[],"keyFacts":[],"issues":[{"issue":"","whyItMatters":"","evidenceNeeded":[]}],"evidenceMentioned":[],"uncertainties":[],"confidence":"high|medium|low"}',
      ].join("\n\n");
      ...
      const decision = createDecision("claim_denial", {
        ...
        decisionTypeLabel: analysis.decision || "SSDI denial", appealInstructions: undefined,
        ...
        issues: (analysis.issues || []).map((item, index) => ({ id: `${index}-${crypto.randomUUID()}`, description: item.issue || "Issue identified in SSDI decision", type: "factual_dispute", severity: "medium", sourceExcerpt: item.whyItMatters })),
