  ...
  server: {
    ...
    try {
      ...
      const session = await stripe.checkout.sessions.create({
        ...
        line_items: [{ price_data: { currency: "usd", product_data: { name: LABELS[mailingMethod], description: `Unemployment denial appeal — ${LABELS[mailingMethod]}` }, unit_amount: PRICES[mailingMethod] }, quantity: 1 }],
        metadata: { appeal_id: appeal.id, workflow_id: "unemployment-denial", mailing_method: mailingMethod, recipient_name: appeal.packet.recipientName, owner_user_id: user.id },
        success_url: `${appUrl}/workflows/unemployment-denial?checkout=success&session_id={CHECKOUT_SESSION_ID}`,
        cancel_url: `${appUrl}/workflows/unemployment-denial?checkout=cancelled`,
      });
