...
async function resolveGemini(task: "analysis") {
  ...
  const response = await fetch(`${base.replace(/\/$/, "")}/api/control-plane/ai`, {
    ...
    body: JSON.stringify({ verticalSlug: "appeal-mail", workflowSlug: "financial-aid-appeal", task }),
  });
...
export const Route = createFileRoute("/api/workflows/financial-aid-appeal/analyze")({
  server: {
    handlers: {
      POST: async ({ request }) => {
    try {
      ...
      const prompt = [
        ...
        "Analyze the actual financial-aid decision or notice. Extract only supported facts and clearly separate uncertainty.",
        "Identify the decision reason, aid type, school/program, deadlines, appeal instructions, special circumstances, academic/progress issues, dependency/financial factors, and evidence needed.",
        "Do not invent institutional policy, FAFSA facts, academic records, financial figures, deadlines, or approval outcomes.",
        "Return strict JSON only.",
        '{"summary":"","decision":"","decisionType":"financial_aid_appeal","issuer":"","referenceNumber":"","decisionDate":"","deadline":"","reasons":[],"keyFacts":[],"issues":[{"issue":"","whyItMatters":"","evidenceNeeded":[]}],"evidenceMentioned":[],"uncertainties":[],"confidence":"high|medium|low"}',
      ].join("\n\n");
      ...
      const decision = createDecision("claim_denial", {
        ...
        decisionTypeLabel: analysis.decision || "Financial aid decision", appealInstructions: undefined,
        ...
        issues: (analysis.issues || []).map((item, index) => ({ id: `${index}-${crypto.randomUUID()}`, description: item.issue || "Issue identified in financial-aid decision", type: "factual_dispute", severity: "medium", sourceExcerpt: item.whyItMatters })),
      ...
      const grounds = (analysis.issues || []).map((issue, index) => createGround("factual_error", {
        ...
        claim: issue.issue || "Review a financial-aid decision issue",
