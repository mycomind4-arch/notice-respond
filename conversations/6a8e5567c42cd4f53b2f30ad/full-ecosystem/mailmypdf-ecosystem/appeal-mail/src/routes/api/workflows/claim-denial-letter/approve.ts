...
export const Route = createFileRoute("/api/workflows/claim-denial-letter/approve")({
  server: {
    handlers: {
        POST: async ({ request }) => {
    try {
      ...
      if (!appeal.draft?.trim()) return Response.json({ error: "The response must be drafted before approval." }, { status: 409 });
