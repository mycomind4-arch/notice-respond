...
async function resolveGemini() {
  ...
  const response = await fetch(`${base.replace(/\/$/, "")}/api/control-plane/ai`, {
    ...
    body: JSON.stringify({ verticalSlug: "appeal-mail", workflowSlug: "insurance-coverage-denial", task: "analysis" }),
  });
...
export const Route = createFileRoute("/api/workflows/insurance-coverage-denial/analyze")({
  server: {
    handlers: {
      POST: async ({ request }) => {
    try {
      ...
      if (!(file instanceof File)) return Response.json({ error: "A coverage denial document is required." }, { status: 400 });
      ...
      const prompt = [
        ...
        "Analyze the actual insurance coverage denial document. Extract only supported facts and clearly separate uncertainty.",
        "Identify the exact coverage determination, policy or plan language referenced, reason for denial, facts relied on, appeal instructions, deadlines, and evidence that could support reconsideration or appeal.",
        "Do not invent policy terms, law, benefits, deadlines, or coverage rules.",
      ...
      const decision = createDecision("claim_denial", {
        ...
        decisionTypeLabel: analysis.decision || "Insurance coverage denial",
        ...
        issues: (analysis.issues || []).map((item, index) => ({ id: `${index}-${crypto.randomUUID()}`, description: item.issue || "Coverage issue identified", type: "factual_dispute", severity: "medium", sourceExcerpt: item.whyItMatters })),
      ...
      const grounds = (analysis.issues || []).map((issue, index) => createGround("factual_error", {
        ...
        claim: issue.issue || "Review a stated coverage issue",
      ...
      const message = error instanceof Error ? error.message : "Unable to analyze coverage denial.";
