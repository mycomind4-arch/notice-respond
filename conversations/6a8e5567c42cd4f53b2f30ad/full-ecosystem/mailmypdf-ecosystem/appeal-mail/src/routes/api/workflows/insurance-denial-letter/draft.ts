...
async function resolveGemini(task: "draft" | "validation") {
  ...
  const response = await fetch(`${base.replace(/\/$/, "")}/api/control-plane/ai`, { method: "POST", headers: { "content-type": "application/json", authorization: `Bearer ${token}` }, body: JSON.stringify({ verticalSlug: "appeal-mail", workflowSlug: "insurance-denial-letter", task }) });
...
export const Route = createFileRoute("/api/workflows/insurance-denial-letter/draft")({
  server: {
    handlers: {
        POST: async ({ request }) => {
    try {
      ...
      if (appeal.workflow_id !== "insurance-denial-letter") return Response.json({ error: "Appeal workflow mismatch." }, { status: 409 });
      ...
      const draft = input.draftOverride?.trim() || await callGemini(draftConfig, [
        `Create a response to the insurance denial letter for: ${workflow.title}.`, workflow.workflowPrompt,
        `Focus on: ${workflow.focusAreas.join(", ")}.`,
        "Use only supplied facts and the denial letter. Do not invent policy language, law, dates, amounts, exclusions, coverage, or outcomes.",
        "Write a concise, professional response that a human can review and edit.",
        `CASE ANALYSIS:\n${JSON.stringify(analysis)}`,
      ].join("\n\n"));
      const validation = await callGemini(validationConfig, [
        `Audit this denial-letter response for: ${workflow.title}.`,
        "Return strict JSON with valid, issues, unsupportedClaims, missingEvidence, suggestions.",
        "Flag unsupported claims, invented policy/legal language, contradictions, missing evidence, deadline problems, and factual uncertainty.",
      ...
      return Response.json({ ok: true, appealId: appeal.id, draft: finalDraft, validation, draftValidation, provider: "gemini", draftModel: draftConfig.model, validationModel: validationConfig.model });
    } catch (error) {
      const message = error instanceof Error ? error.message : "Unable to create denial-letter response.";
