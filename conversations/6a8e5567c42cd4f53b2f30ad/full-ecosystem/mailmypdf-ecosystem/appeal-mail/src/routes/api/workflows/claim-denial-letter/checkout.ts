...
export const Route = createFileRoute("/api/workflows/claim-denial-letter/checkout")({
  server: {
    handlers: {
      POST: async ({ request }) => {
    try {
      ...
      if (appeal.status !== "ready" || !appeal.review || !appeal.packet) return Response.json({ error: "Response is not approved and ready for payment." }, { status: 409 });
      ...
      const session = await stripe.checkout.sessions.create({
        ...
        line_items: [{ price_data: { currency: "usd", product_data: { name: LABELS[mailingMethod], description: `Claim denial response — ${LABELS[mailingMethod]}` }, unit_amount: PRICES[mailingMethod] }, quantity: 1 }],
        metadata: { appeal_id: appeal.id, workflow_id: "claim-denial-letter", mailing_method: mailingMethod, recipient_name: appeal.packet.recipientName, owner_user_id: user.id },
        success_url: `${appUrl}/workflows/claim-denial-letter?checkout=success&session_id={CHECKOUT_SESSION_ID}`,
        cancel_url: `${appUrl}/workflows/claim-denial-letter?checkout=cancelled`,
      });
