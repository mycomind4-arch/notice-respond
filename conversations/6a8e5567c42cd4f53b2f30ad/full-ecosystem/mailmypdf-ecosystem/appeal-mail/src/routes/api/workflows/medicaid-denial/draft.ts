...
export const Route = createFileRoute("/api/workflows/medicaid-denial/draft")({
  server: {
    handlers: {
        POST: async ({ request }) => {
    try {
      ...
      const draft = input.draftOverride?.trim() || await callGemini(draftConfig, [
        ...
        "Use only supplied facts. Do not invent Medicaid program rules, legal authority, diagnoses, eligibility facts, dates, deadlines, or outcomes.",
        "Write a clear, professional appeal that a human can review and edit.",
        `CASE ANALYSIS:\n${JSON.stringify(analysis)}`,
      ].join("\n\n"));
      const validation = await callGemini(validationConfig, [
        `Audit this Medicaid denial appeal draft for: ${workflow.title}.`,
        "Return strict JSON with valid, issues, unsupportedClaims, missingEvidence, suggestions.",
        "Flag unsupported claims, fabricated program/legal authority, missing evidence, contradictions, deadline problems, and factual uncertainty.",
        `CASE ANALYSIS:\n${JSON.stringify(analysis)}`,
        `DRAFT:\n${draft}`,
      ].join("\n\n"));
