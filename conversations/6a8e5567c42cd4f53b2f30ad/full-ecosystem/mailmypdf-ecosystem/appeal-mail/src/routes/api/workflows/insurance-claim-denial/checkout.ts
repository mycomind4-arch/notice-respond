  ...
  server: {
    ...
    try {
      ...
      const session = await stripe.checkout.sessions.create({
        ...
        line_items: [{ price_data: { currency: "usd", product_data: { name: LABELS[mailingMethod], description: `Insurance claim denial appeal — ${LABELS[mailingMethod]}` }, unit_amount: PRICES[mailingMethod] }, quantity: 1 }],
        metadata: { appeal_id: appeal.id, workflow_id: "insurance-claim-denial", mailing_method: mailingMethod, owner_user_id: user.id },
