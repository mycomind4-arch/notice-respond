...
export const Route = createFileRoute("/api/workflows/sap-appeal/draft")({server:{handlers:{ POST: async ({ request }) => {
  try {
    ...
    const draft = input.draftOverride?.trim() || await callGemini(d, [
      ...
      `Focus on: ${wf.focusAreas.join(", ")}.`,
      "Use only supplied facts. Do not invent school policy, academic metrics, diagnoses, deadlines, or documentation.",
      "Write a professional SAP appeal a student can review and edit. Address the stated SAP problem, documented circumstances, supporting evidence, and a credible recovery plan when supported by the record.",
      `CASE ANALYSIS:\n${JSON.stringify(analysis)}`,
    ].join("\n\n"));
    const validation = await callGemini(v, [
      `Audit this ${wf.title} draft.`,
      "Return strict JSON with valid, issues, unsupportedClaims, missingEvidence, suggestions.",
      "Flag invented school policy, academic figures, circumstances, documentation, deadlines, contradictions, and uncertainty.",
