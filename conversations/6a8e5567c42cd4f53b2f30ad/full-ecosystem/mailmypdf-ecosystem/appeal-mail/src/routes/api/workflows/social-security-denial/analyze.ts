...
async function resolveGemini() {
  ...
  const response = await fetch(`${base.replace(/\/$/, "")}/api/control-plane/ai`, {
    ...
    body: JSON.stringify({ verticalSlug: "appeal-mail", workflowSlug: "social-security-denial", task: "analysis" }),
  });
...
export const Route = createFileRoute("/api/workflows/social-security-denial/analyze")({
  server: {
    handlers: {
      POST: async ({ request }) => {
    try {
      ...
      const prompt = [
        ...
        "Analyze the actual Social Security decision. Extract only supported facts and clearly identify uncertainty.",
        "Identify decision findings, stated reasons, deadlines, appeal/reconsideration instructions, evidence considered, evidence gaps, and disputed facts.",
        "Do not invent medical facts, SSA rules, legal authority, dates, deadlines, or outcomes.",
        "Return strict JSON only.",
        '{"summary":"","decision":"","decisionType":"social_security_denial","issuer":"","referenceNumber":"","decisionDate":"","deadline":"","reasons":[],"keyFacts":[],"issues":[{"issue":"","whyItMatters":"","evidenceNeeded":[]}],"evidenceMentioned":[],"uncertainties":[],"confidence":"high|medium|low"}'
      ].join("\n\n");
      ...
      const decision = createDecision("claim_denial", {
        id: crypto.randomUUID(), documentId: document.id, documentFilename: document.filename, agency: analysis.issuer || "Social Security Administration",
        referenceNumber: analysis.referenceNumber || undefined, decisionDate: analysis.decisionDate || undefined,
        decisionTypeLabel: analysis.decision || "Social Security decision", appealInstructions: undefined,
        deadline: analysis.deadline ? { date: analysis.deadline, type: "appeal", source: "extracted" } : undefined,
        facts: (analysis.keyFacts || []).map((value: string, index: number) => ({ id: `${index}-${crypto.randomUUID()}`, label: `Fact ${index + 1}`, value, source: "extracted", confidence: 0.8 })),
        reasons: (analysis.reasons || []).map((text: string, index: number) => ({ id: `${index}-${crypto.randomUUID()}`, text, confidence: 0.9 })),
        issues: (analysis.issues || []).map((item: any, index: number) => ({ id: `${index}-${crypto.randomUUID()}`, description: item.issue || "Issue identified in decision", type: "factual_dispute", severity: "medium", sourceExcerpt: item.whyItMatters })),
