...
async function resolveGemini() {
  ...
  const r = await fetch(`${base.replace(/\/$/, "")}/api/control-plane/ai`, {
    ...
    body: JSON.stringify({ verticalSlug: "appeal-mail", workflowSlug: "sap-appeal", task: "analysis" }),
  });
...
export const Route = createFileRoute("/api/workflows/sap-appeal/analyze")({server:{handlers:{POST:async ({ request }) => {
  try {
    const user = await requireAuthenticatedUser(request);
    const workflow = getWorkflow("sap-appeal");
    const form = await request.formData();
    const file = form.get("document");
    if (!(file instanceof File)) return Response.json({ error: "An SAP decision or financial-aid notice is required." }, { status: 400 });
    ...
    const prompt = [
      ...
      "Analyze the actual satisfactory-academic-progress (SAP) decision or related financial-aid notice.",
      "Extract only supported facts and distinguish the school's stated findings from student assertions or uncertainty.",
      "Identify SAP status, qualitative/quantitative standards mentioned, attempted/completed credits, GPA or pace figures when stated, maximum timeframe when stated, decision date, appeal deadline, instructions, special-circumstance information, documentation referenced, and recovery-plan requirements.",
      "Do not invent school policy, numbers, deadlines, diagnoses, or facts not contained in the document.",
      "Return strict JSON only.",
      '{"summary":"","decision":"","decisionType":"sap_status","school":"","referenceNumber":"","decisionDate":"","deadline":"","sapStatus":"","standardsCited":[],"metricsStated":[],"specialCircumstancesMentioned":[],"documentationMentioned":[],"appealInstructions":"","recoveryPlanRequirements":[],"issues":[{"issue":"","whyItMatters":"","evidenceNeeded":[]}],"uncertainties":[],"confidence":"high|medium|low"}'
    ].join("\n\n");

    const r = await fetch(`https://generativelanguage.googleapis.com/v1beta/models/${encodeURIComponent(gemini.model)}:generateContent?key=${encodeURIComponent(gemini.apiKey)}`, {
      method: "POST",
    ...
    const text = body?.candidates?.[0]?.content?.parts?.map((x: any) => x.text || "").join("").trim();
    ...
    const decision = createDecision("claim_denial", {
      ...
      agency: analysis.school || undefined,
      referenceNumber: analysis.referenceNumber || undefined,
      decisionDate: analysis.decisionDate || undefined,
      decisionTypeLabel: analysis.decision || "SAP decision",
      deadline: analysis.deadline ? { date: analysis.deadline, type: "appeal", source: "extracted" } : undefined,
      facts: [ ...(analysis.metricsStated || []), ...(analysis.specialCircumstancesMentioned || []) ].map((value: string, i: number) => ({ id: `${i}-${crypto.randomUUID()}`, label: `Fact ${i + 1}`, value, source: "extracted", confidence: 0.8 })),
      reasons: (analysis.standardsCited || []).map((text: string, i: number) => ({ id: `${i}-${crypto.randomUUID()}`, text, confidence: 0.9 })),
      issues: (analysis.issues || []).map((x: any, i: number) => ({ id: `${i}-${crypto.randomUUID()}`, description: x.issue || "SAP appeal issue", type: "factual_dispute", severity: "medium", sourceExcerpt: x.whyItMatters })),
    ...
    const grounds = (analysis.issues || []).map((x: any, i: number) => createGround("factual_error", {
      id: `ground-${i}-${crypto.randomUUID()}`,
      claim: x.issue || "Review the SAP determination",
      source: x.whyItMatters || "Identified by document analysis",
      confidence: 0.65,
    ...
    const message = error instanceof Error ? error.message : "Unable to analyze SAP decision.";
