...
export default defineEventHandler(async (event) => {
  ...
  const { data: intent, error: intentError } = await supabase.from("mailing_intents").insert({
    user_id: user.id,
  ...
  try {
    ...
    const { error: updateError } = await supabase.from("mailing_intents").update({ stripe_session_id: session.id }).eq("id", intent.id).eq("user_id", user.id);
    if (updateError) throw updateError;
    return { ok: true, checkoutUrl: session.url, sessionId: session.id };
  } catch (error) {
    await supabase.from("mailing_intents").update({ status: "failed", error_message: error instanceof Error ? error.message : "Stripe checkout creation failed" }).eq("id", intent.id).eq("user_id", user.id);
