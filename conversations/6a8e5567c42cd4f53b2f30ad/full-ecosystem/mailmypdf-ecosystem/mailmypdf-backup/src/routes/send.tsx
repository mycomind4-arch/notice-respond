...
export const Route = createFileRoute("/send")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Upload a PDF, enter the addresses, and pay to have your letter mailed." },
    ],
  }),
  component: SendPage,
});
...
function SendPage() {
  ...
  const [file, setFile] = useState<{ name: string; sizeBytes: number; pages: number; priceCents: number } | null>(null);
          ...
          <li key={label} className="flex flex-1 items-center gap-3">
            <button
              ...
              className={`flex h-9 w-9 shrink-0 items-center justify-center rounded-full border font-mono text-xs transition-colors ${
                active
                  ? "border-stamp bg-stamp text-accent-foreground"
                  : done
...
function UploadStep({
  ...
  onFile: (f: { name: string; sizeBytes: number; pages: number; priceCents: number } | null, raw: File | null) => void;
  previewFn: (args: { data: { sizeBytes: number; dataBase64: string } }) => Promise<{ pageCount: number; priceCents: number } | { error: string }>;
  error: string | null;
  ...
  const handleFiles = async (files: FileList | null) => {
    ...
    try {
      ...
      onFile({ name: f.name, sizeBytes: f.size, pages: result.pageCount, priceCents: result.priceCents }, f);
    } catch (e) {
  ...
  return (
    <section>
      ...
      <p className="mt-2 text-muted-foreground">PDF only, up to 10 pages and 10MB. Color and black-and-white options available at checkout.</p>
      ...
      <label
        ...
        className={`mt-8 flex cursor-pointer flex-col items-center justify-center rounded-lg border-2 border-dashed p-12 text-center transition-colors ${
          dragging ? "border-stamp bg-stamp/5" : "border-rule bg-card"
        }`}
      >
      ...
      {error && <div className="mt-4 rounded-md border border-destructive/40 bg-destructive/5 px-4 py-3 text-sm text-destructive">{error}</div>}
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <div className="font-mono text-xs uppercase tracking-widest text-muted-foreground">
                {file.pages} page{file.pages === 1 ? "" : "s"} · {(file.sizeBytes / 1024).toFixed(0)} KB · ${(file.priceCents / 100).toFixed(2)}
              </div>
            </div>
          </div>
          <button
      ...
      <div className="mt-8 flex justify-end">
        <button
          ...
          disabled={!file}
          onClick={onNext}
          className="inline-flex items-center gap-2 rounded-full bg-primary px-6 py-3 text-sm font-medium text-primary-foreground disabled:cursor-not-allowed disabled:opacity-40 hover:opacity-90"
        >
...
function AddressStep({
  email, setEmail, sender, setSender, recipient, setRecipient, onBack, onNext, error,
}: {
  email: string;
...
}: { title: string; subtitle: string; value: Address; onChange: (a: Address) => void }) {
  ...
  return (
    <div className="envelope-card p-6">
      ...
      <div className="mt-4 space-y-4">
        <Field label="Full name">{(id) => <input id={id} value={value.name} onChange={set("name")} className={inputClasses} />}</Field>
        <Field label="Address line 1">{(id) => <input id={id} value={value.line1} onChange={set("line1")} className={inputClasses} />}</Field>
        <Field label="Address line 2 (optional)">{(id) => <input id={id} value={value.line2} onChange={set("line2")} className={inputClasses} />}</Field>
        <div className="grid grid-cols-3 gap-3">
          <Field label="City">{(id) => <input id={id} value={value.city} onChange={set("city")} className={inputClasses} />}</Field>
          <Field label="State">{(id) => <input id={id} value={value.state} onChange={set("state")} maxLength={2} placeholder="CA" className={`${inputClasses} uppercase`} />}</Field>
          <Field label="ZIP">{(id) => <input id={id} value={value.postalCode} onChange={set("postalCode")} placeholder="94105" className={inputClasses} />}</Field>
        </div>
      </div>
    </div>
  );
...
const inputClasses =
  "mt-1.5 w-full rounded-md border border-input bg-background px-3 py-2 text-sm text-foreground outline-none focus:border-ink focus:ring-2 focus:ring-ink/10";

/* -------------------- Step 3: Review -------------------- */

function ReviewStep({
  file, sender, recipient, email, priceCents,
  color, setColor, mailClass, setMailClass,
  agreedReviewed, setAgreedReviewed, agreedContent, setAgreedContent,
  onBack, onNext, submitting, error,
}: {
    ...
    <section>
      ...
      <div className="mt-8 envelope-card envelope-card-notch p-8">
        <div className="grid gap-8 md:grid-cols-2">
          <div>
            <div className="text-[10px] uppercase tracking-[0.25em] text-muted-foreground">Document</div>
            ...
            <div className="mt-6 text-[10px] uppercase tracking-[0.25em] text-muted-foreground">Confirmation email</div>
      ...
      <label className="mt-6 flex cursor-pointer items-start gap-3 rounded-md border border-input bg-card p-4 text-sm">
        ...
        <span className="text-muted-foreground">
          I reviewed my document and both addresses. I understand MailMyPDF will print and mail
          this document exactly as submitted, with no edits or refunds after payment.
        </span>
      </label>
      ...
      <label className="mt-3 flex cursor-pointer items-start gap-3 rounded-md border border-input bg-card p-4 text-sm">
        ...
        />
        <span className="text-muted-foreground">
          I agree not to use MailMyPDF to send unlawful, threatening, fraudulent, abusive, or
          spam content, and I'm authorized to send this document.
        </span>
      </label>
      ...
      <div className="mt-8 flex items-center justify-between">
        ...
        <button
          ...
          className="inline-flex items-center gap-2 rounded-full bg-primary px-6 py-3 text-sm font-medium text-primary-foreground disabled:cursor-not-allowed disabled:opacity-40 hover:opacity-90"
        >
          {submitting ? "Preparing checkout…" : <>Continue to payment <Arrow /></>}
        </button>
      </div>
    </section>
  );
...
function AddressBlock({ a }: { a: Address }) {
  return (
    <div className="mt-1 font-mono text-sm leading-relaxed text-foreground">
      ...
      <div>{a.city}{a.city && ","} {a.state?.toUpperCase()} {a.postalCode}</div>
    </div>
  );
}
  ...
  return (
    <section>
      ...
      <p className="mx-auto mt-2 max-w-md text-center text-muted-foreground">
        Card details are collected and processed by our payment provider — MailMyPDF never sees them.
      </p>
...
function PdfIcon() {
  return (
    <div className="flex h-12 w-10 items-center justify-center rounded-sm border border-rule bg-paper-deep font-mono text-[10px] tracking-widest text-stamp">
      PDF
    </div>
  );
}
