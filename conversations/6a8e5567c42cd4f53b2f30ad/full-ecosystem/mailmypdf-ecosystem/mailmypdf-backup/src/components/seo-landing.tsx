...
const DEFAULT_FAQ = [
  { q: "What file types do you accept?", a: "PDF only in this first version. Files must be under 10MB and no more than 10 pages." },
  { q: "Do I need an account?", a: "No. You can upload, pay, and send as a guest. Your order status link will be sent by email." },
  { q: "Is certified mail available?", a: "Yes. You can add Certified Mail (tracking + signature) or Registered Mail (insured + tracking) at checkout for an additional fee." },
  { q: "Can I send international mail?", a: "Not yet. MailMyPDF currently supports U.S. domestic mail only." },
  { q: "Do you read my document?", a: "No. Uploaded PDFs are processed for printing and mailing. We do not use uploaded documents for AI training." },
...
const STEPS = [
  ...
  { n: "02", t: "Enter the recipient's mailing address", d: "U.S. domestic addresses only in this release." },
        ...
        <div className="mx-auto grid max-w-6xl gap-8 px-6 py-16 md:grid-cols-2">
          <div className="envelope-card p-8">
            ...
            <p className="mt-3 text-muted-foreground">
              Simple per-letter pricing starts at $4.99 for 1–2 page black-and-white U.S. domestic letters.
            </p>
            <div className="mt-4">
              <Link to="/" hash="pricing" className="text-sm text-stamp underline-offset-4 hover:underline">
          ...
          <div className="envelope-card p-8">
            ...
            <p className="mt-3 text-muted-foreground">
              Uploaded PDFs are used only to process, print, and mail your order. We do not use customer
              documents for AI training, resale, or marketing.
            </p>
          </div>
        </div>
      </section>
            ...
            <div className="mt-6 grid gap-3 md:grid-cols-3">
              {props.relatedLinks.map((r) => (
                <Link
                  key={r.to}
                  to={r.to}
                  className="envelope-card block px-5 py-4 text-sm font-medium text-foreground hover:text-stamp"
                >
                  {r.label} →
                </Link>
              ))}
