...
const FROM = "MailMyPDF <onboarding@resend.dev>";
const SUPPORT_EMAIL = "support@mailmypdf.com";
...
export async function sendOrderRecoveryEmail(
  ...
  const html = `<div style="font-family:Georgia,serif;max-width:560px;margin:auto;padding:24px;color:#1c1917">
    ...
    <p style="font-size:12px;color:#666;margin-top:20px">If you didn't request this, you can safely ignore it. Questions? Contact ${SUPPORT_EMAIL}.</p>
  </div>`;
  const result = await sendViaResend({
    to: email,
