...
import { createCommunication, getCommunication, type CreateNoticeCommunicationInput, type MailType } from "./mailmypdf";
...
export class MailMyPDFProvider implements MailingProvider {
  async createLetter(input: MailingOrderDraft): Promise<{ providerOrderId: string }> {
    ...
    const communicationInput: CreateNoticeCommunicationInput = {
      ...
      matter_type: input.matterType ?? "notice-respond",
      metadata: {
