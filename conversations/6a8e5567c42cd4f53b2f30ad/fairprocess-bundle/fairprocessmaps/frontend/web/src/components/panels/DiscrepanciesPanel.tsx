...
function ruleLabel(finding: Finding) {
  ...
  const labels: Record<string, string> = {
    notice_timing: "Was Adequate Notice Provided?",
    hearing_right: "Was a Hearing Opportunity Available?",
    appeal_pathway: "Was an Appeal Pathway Available?",
    abatement_without_notice: "Was Notice Provided Before Abatement?",
    permit_review_right: "Were Permit Review Rights Honored?",
    ce_outcome_review: "Was the Code Enforcement Outcome Reviewed?",
  };
...
export default function DiscrepanciesPanel({ projectId }: { projectId: string }) {
  ...
  const missingInfoFindings = findings.filter(f => f.status === "open" && (f.missing_info === 1 || f.missing_info === true));
  ...
  function scoreLabel(s: number | null) {
    if (s === null) return "Not yet assessed";
    if (s >= 80) return "Few procedural concerns — review recommended";
    if (s >= 60) return "Several items warrant review";
    return "Multiple items warrant review — prioritize";
  }
      ...
      <div className="rounded-xl border border-fp-amber/30 bg-fp-amber/10 p-4 flex items-start gap-3">
        ...
        <div>
          ...
          <p className="text-xs text-fp-text-muted mt-1 leading-relaxed">
            These findings are automated procedural flags generated by the Rules Engine for human review.
            They raise questions about due process compliance but do not constitute legal determinations,
            conclusions, or legal advice. Always consult a qualified attorney for legal guidance.
          </p>
        </div>
      </div>
