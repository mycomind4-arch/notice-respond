...
def generate(repo_path, product_name, accent_color, workflow_import_path, workflow_type_id_type, engine_import_path=None):
    os.makedirs(f"{repo_path}/app/workflows/[id]", exist_ok=True)
    os.makedirs(f"{repo_path}/app", exist_ok=True)
    
    # package.json
    pkg = {"name":product_name.lower().replace(' ','-'),"version":"0.1.0","private":True,
           "scripts":{"dev":"next dev","build":"next build","start":"next start",
                      "typecheck":"tsc --noEmit","test":"vitest run","test:watch":"vitest","lint":"next lint"},
           "dependencies":{"lucide-react":"^0.511.0","next":"^15.3.0","react":"^19.1.0","react-dom":"^19.1.0"},
           "devDependencies":{"@types/node":"^22.15.0","@types/react":"^19.1.0","@types/react-dom":"^19.1.0",
                             "typescript":"^5.8.0","vitest":"^3.2.4"}}
    with open(f"{repo_path}/package.json","w") as f: json.dump(pkg,f,indent=2); f.write("\n")
    
    # tsconfig.json
    ts = {"compilerOptions":{"target":"ES2017","lib":["dom","dom.iterable","esnext"],"allowJs":True,
    ...
    with open(f"{repo_path}/tsconfig.json","w") as f: json.dump(ts,f,indent=2); f.write("\n")
    ...
    with open(f"{repo_path}/app/globals.css","w") as f:
        f.write(f"""*{{box-sizing:border-box}}
...
.card:hover{{border-color:rgba({ac},.25);background:rgba(255,255,255,.06)}}
.muted{{color:#94a3b8}}
.eyebrow{{font-size:11px;font-weight:800;letter-spacing:.16em;text-transform:uppercase;color:#{ac}}}
...
.btn-primary{{background:#{ac};color:#061018}}
...
.badge-family{{background:rgba({ac},.1);color:#{ac};border:1px solid rgba({ac},.2)}}
...
.hero-gradient{{background:radial-gradient(ellipse at top,rgba({ac},.08) 0%,transparent 60%),radial-gradient(ellipse at bottom right,rgba({ac},.05) 0%,transparent 50%)}}
...
.pipeline-step:hover{{border-color:rgba({ac},.2);background:rgba({ac},.03)}}
.pipeline-num{{font-size:12px;font-weight:800;color:#{ac};min-width:24px}}
...
.evidence-list li::before{{content:"";display:inline-block;width:6px;height:6px;border-radius:50%;background:#{ac};margin-right:12px;vertical-align:middle}}
...
export const metadata = {{
  ...
  openGraph: {{ title: '{product_name}', description: '{product_name} — focused workflows.', type: 'website', siteName: '{product_name}' }},
  twitter: {{ card: 'summary_large_image', title: '{product_name}', description: '{product_name} — focused workflows.' }},
...
export default function RootLayout({{ children }}: {{ children: React.ReactNode }}) {{
  return (
    ...
    # Read the template files and substitute
    tmpl_dir = os.path.join(os.path.dirname(__file__), 'templates')
    
    # page.tsx (home)
    with open(f"{repo_path}/app/page.tsx","w") as f:
        f.write(_home_page(product_name, ac, workflow_import_path))
    
    # workflows/page.tsx (hub)
    with open(f"{repo_path}/app/workflows/page.tsx","w") as f:
        f.write(_hub_page(product_name, ac, workflow_import_path))
    
    # workflows/[id]/page.tsx (individual)
    with open(f"{repo_path}/app/workflows/[id]/page.tsx","w") as f:
        f.write(_detail_page(product_name, ac, workflow_import_path, workflow_type_id_type, engine_import_path))
    
    print(f"✓ Generated {product_name} at {repo_path}")
    
    # Post-process: fix any remaining template variables in generated files
    import glob
    for filepath in glob.glob(f"{repo_path}/app/**/*.tsx", recursive=True):
        with open(filepath) as tf:
            content = tf.read()
        content = content.replace('{pn_lower}', product_name.lower())
        content = content.replace('{pn}', product_name)
        with open(filepath, 'w') as tf:
            tf.write(content)

def _home_page(pn, ac, imp):
    pn_l = pn.lower()
    return f"""import Link from 'next/link'
import {{ WORKFLOWS, type WorkflowFamily }} from '{imp}'

export const metadata = {{
  title: '{pn} | Decision, Denial & Appeal Workflows',
  description: '{pn} — focused workflows for responding to government decisions, denials, and notices. Upload documents, organize evidence, draft a response, review, and mail with proof.',
}}
    ...
    <main>
      <header style={{{{borderBottom:'1px solid rgba(255,255,255,.08)',position:'sticky',top:0,background:'rgba(6,16,24,.85)',backdropFilter:'blur(12px)',zIndex:100}}}}>
        <div className="container" style={{{{display:'flex',justifyContent:'space-between',alignItems:'center',padding:'18px 0'}}}}>
          <Link href="/" style={{{{fontWeight:800,fontSize:18,letterSpacing:'-.02em'}}}}>{pn}</Link>
      ...
      <section className="hero-gradient" style={{{{borderBottom:'1px solid rgba(255,255,255,.08)'}}}}>
        <div className="container" style={{{{paddingTop:88,paddingBottom:88}}}}>
          ...
          <div style={{{{display:'flex',gap:32,marginTop:48,flexWrap:'wrap'}}}}>
            <div><div style={{{{fontSize:32,fontWeight:800,color:'#{ac}'}}}}>{{totalWorkflows}}</div><div className="muted" style={{{{fontSize:13}}}}>Focused workflows</div></div>
            <div><div style={{{{fontSize:32,fontWeight:800,color:'#{ac}'}}}}>{{families.length}}</div><div className="muted" style={{{{fontSize:13}}}}>Categories</div></div>
            <div><div style={{{{fontSize:32,fontWeight:800,color:'#{ac}'}}}}>{{(totalSearchVolume/1000).toFixed(0)}}K</div><div className="muted" style={{{{fontSize:13}}}}>Monthly search volume</div></div>
            <div><div style={{{{fontSize:32,fontWeight:800,color:'#{ac}'}}}}>8</div><div className="muted" style={{{{fontSize:13}}}}>Pipeline stages</div></div>
          </div>
        </div>
      </section>
          ...
          <div style={{{{display:'grid',gap:16,gridTemplateColumns:'repeat(auto-fit,minmax(240px,1fr))'}}}}>
            {{[
              ...
              <div className="card" style={{{{padding:24}}}} key={{n}}>
                <div style={{{{fontSize:12,fontWeight:800,color:'#{ac}'}}}}>{{n}}</div>
              ...
              return (
                <Link key={{family}} href={{`/workflows?family=${{encodeURIComponent(family)}}`}} className="card" style={{{{padding:28,display:'block'}}}}>
                  ...
                  <div style={{{{marginTop:16,display:'flex',flexWrap:'wrap',gap:8}}}}>
                    ...
                    {{workflows.length > 4 && <span style={{{{fontSize:12,padding:'4px 10px',color:'#{ac}'}}}}>+{{workflows.length - 4}} more</span>}}
                  </div>
                </Link>
              )
            }})}}
            ...
            {{WORKFLOWS.slice().sort((a,b) => b.monthlySearches - a.monthlySearches).slice(0,9).map(w => (
              <Link key={{w.id}} href={{`/workflows/${{w.id}}`}} className="card" style={{{{padding:24,display:'block'}}}}>
                ...
                <div style={{{{display:'flex',gap:16,marginTop:16}}}}>
                  <span style={{{{fontSize:12,color:'#{ac}'}}}}>{{w.monthlySearches.toLocaleString()}}/mo</span>
      ...
      <section id="status" className="section" style={{{{borderTop:'1px solid rgba(255,255,255,.08)'}}}}>
        <div className="container">
          <div className="card" style={{{{padding:36,borderColor:'rgba(251,191,36,.2)',background:'rgba(251,191,36,.03)'}}}}>
            ...
            <p style={{{{margin:0,maxWidth:720,fontSize:15,lineHeight:1.7,color:'#94a3b8'}}}}>These pages are the master information architecture for {pn}. We are not presenting unfinished workflows as fully operational. Each workflow will become executable as its product implementation is migrated into this repository.</p>
          </div>
        </div>
      </section>
      ...
      <footer style={{{{borderTop:'1px solid rgba(255,255,255,.08)',padding:'40px 0'}}}}>
        <div className="container" style={{{{display:'flex',justifyContent:'space-between',alignItems:'center',flexWrap:'wrap',gap:16}}}}>
          <div><div style={{{{fontWeight:800,fontSize:16}}}}>{pn}</div><div style={{{{fontSize:13,color:'#94a3b8',marginTop:4}}}}>{{totalWorkflows}} workflows · {{families.length}} categories</div></div>
          <div style={{{{fontSize:13,color:'#64748b'}}}}>{pn} is not a law firm and does not provide legal advice.</div>
        </div>
      </footer>
    </main>
  )
...
def _hub_page(pn, ac, imp):
    slug = pn.lower().replace(' ','-')
    return f"""import Link from 'next/link'
import {{ WORKFLOWS, type WorkflowFamily }} from '{imp}'

const SITE_ORIGIN = 'https://{slug}.pages.dev'

export function generateMetadata() {{
  return {{
    title: `{pn} Workflows — All | {pn}`,
    description: `Browse all {pn} workflows organized by category. Upload the decision or notice, organize evidence, draft a response, review, and mail with proof.`,
    openGraph: {{ title: `{pn} Workflows`, description: 'Problem-specific workflows with document upload, evidence organization, response drafting, review, and mailing proof.', type: 'website', siteName: '{pn}', url: SITE_ORIGIN + '/workflows' }},
    other: {{ 'application/ld+json': JSON.stringify({{ '@context': 'https://schema.org', '@type': 'ItemList', name: '{pn} Workflows', itemListElement: WORKFLOWS.map((w, i) => ({{ '@type': 'ListItem', position: i + 1, name: w.name, url: SITE_ORIGIN + '/workflows/' + w.id }})) }}) }},
  }}
}}
...
export default function WorkflowsHub() {{
  ...
  return (
    <main>
                ...
                {{workflows.map(w => (
                  <Link key={{w.id}} href={{`/workflows/${{w.id}}`}} className="card" style={{{{padding:22,display:'block'}}}}>
                    ...
                    <div style={{{{display:'flex',gap:14,marginTop:14,paddingTop:14,borderTop:'1px solid rgba(255,255,255,.06)'}}}}>
                      <span style={{{{fontSize:12,color:'#{ac}',fontWeight:600}}}}>{{w.primaryKeyword}}</span>
      ...
      <footer style={{{{borderTop:'1px solid rgba(255,255,255,.08)',padding:'40px 0'}}}}>
        <div className="container" style={{{{display:'flex',justifyContent:'space-between',alignItems:'center',flexWrap:'wrap',gap:16}}}}>
          <div><Link href="/" style={{{{fontWeight:800,fontSize:16}}}}>{pn}</Link><div style={{{{fontSize:13,color:'#94a3b8',marginTop:4}}}}>{{WORKFLOWS.length}} workflows</div></div>
          <div style={{{{fontSize:13,color:'#64748b'}}}}>{pn} is not a law firm and does not provide legal advice.</div>
        </div>
      </footer>
    </main>
...
def _detail_page(pn, ac, imp, id_type, engine_imp):
    slug = pn.lower().replace(' ','-')
    stages_import = f"import {{ STAGES }} from '{engine_imp}'\n" if engine_imp else ""
    return f"""import Link from 'next/link'
import {{ notFound }} from 'next/navigation'
{stages_import}import {{ workflowMap, WORKFLOWS, type {id_type} }} from '{imp}'
...
export async function generateMetadata({{ params }}: {{ params: Promise<{{ id: string }}> }}) {{
  ...
  const workflow = workflowMap[id as {id_type}]
  if (!workflow) return {{ title: '{pn} Workflow' }}
  return {{
    title: `${{workflow.name}} | {pn}`,
    description: workflow.description,
    keywords: [workflow.primaryKeyword, ...workflow.supportingKeywords],
    alternates: {{ canonical: `/workflows/${{workflow.id}}` }},
    openGraph: {{ title: `${{workflow.name}} | {pn}`, description: workflow.description, type: 'article', siteName: '{pn}', url: `${{SITE_ORIGIN}}/workflows/${{workflow.id}}` }},
    twitter: {{ card: 'summary', title: `${{workflow.name}} | {pn}`, description: workflow.description }},
  ...
  return (
    <main>
      <header style={{{{borderBottom:'1px solid rgba(255,255,255,.08)',position:'sticky',top:0,background:'rgba(6,16,24,.85)',backdropFilter:'blur(12px)',zIndex:100}}}}>
        <div className="container" style={{{{display:'flex',justifyContent:'space-between',alignItems:'center',padding:'18px 0'}}}}>
          <Link href="/" style={{{{fontWeight:800,fontSize:18,letterSpacing:'-.02em'}}}}>{pn}</Link>
          <nav style={{{{display:'flex',gap:28,fontSize:14,color:'#94a3b8'}}}}><Link href="/workflows">All workflows</Link><Link href="/">Home</Link></nav>
        </div>
      </header>
      ...
      <section style={{{{paddingTop:32,paddingBottom:40}}}}>
        <div className="container">
          ...
          <div style={{{{display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(160px,1fr))',gap:14,maxWidth:680}}}}>
            ...
            <div className="stat-card"><div className="muted" style={{{{fontSize:12}}}}>Primary keyword</div><div style={{{{fontSize:15,fontWeight:600,marginTop:6,color:'#{ac}'}}}}>{{workflow.primaryKeyword}}</div></div>
          </div>
        </div>
      </section>
      ...
      <section className="section-tight" style={{{{borderTop:'1px solid rgba(255,255,255,.08)'}}}}>
        <div className="container">
          ...
          <div style={{{{display:'flex',flexWrap:'wrap',gap:10}}}}>
            <span style={{{{padding:'8px 16px',borderRadius:10,background:'rgba({ac},.1)',color:'#{ac}',fontSize:13,fontWeight:600}}}}>{{workflow.primaryKeyword}} ({{workflow.monthlySearches.toLocaleString()}}/mo)</span>
...
# ── Build all repos ──
builds = json.loads(sys.argv[1]) if len(sys.argv) > 1 else []
...
for b in builds:
    generate(
        ...
        workflow_import_path=b['workflow_import_path'],
        workflow_type_id_type=b['workflow_type_id_type'],
        engine_import_path=b.get('engine_import_path')
    )
