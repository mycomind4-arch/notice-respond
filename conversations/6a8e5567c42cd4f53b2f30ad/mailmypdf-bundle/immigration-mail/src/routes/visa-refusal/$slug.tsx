...
import { findVisaPage } from "@/domain/visa-refusal-content";

export const Route = createFileRoute('/visa-refusal/$slug')({
  head: ({ params }) => {
    const page = findVisaPage(`/visa-refusal/${params.slug}`);
    if (!page) {
  ...
  component: VisaRefusalSupportingPage,
});

function VisaRefusalSupportingPage() {
  const { slug } = Route.useParams();
  const page = findVisaPage(`/visa-refusal/${slug}`);
      ...
      <div className="min-h-screen page-fade">
        ...
        <main>
          <div className="mx-auto max-w-4xl px-6 py-24 text-center">
            ...
            <p className="mt-4 text-ink-soft">This visa refusal resource page doesn't exist.</p>
            <Link to="/visa-refusal" className="mt-6 inline-flex rounded-full bg-primary px-6 py-3 text-primary-foreground">
              Back to Visa Refusal hub
            </Link>
          </div>
        </main>
        <SiteFooter />
              ...
              {page.breadcrumbs.map((crumb, idx) => (
                <span key={idx} className="flex items-center gap-2">
                  ...
                  {idx < page.breadcrumbs.length - 1 ? (
                    <Link to={crumb.path === '/' ? '/' : crumb.path.startsWith('/visa-refusal/') ? '/visa-refusal/$slug' : '/visa-refusal'} params={crumb.path === '/' ? {} : { slug: crumb.path.replace('/visa-refusal/', '') }} className="hover:text-ink">
                      {crumb.label}
                    </Link>
                  ) : (
          ...
          <div className="mx-auto max-w-4xl px-6 py-12">
            <div className="envelope-card p-8 text-center">
              <h2 className="text-xl md:text-2xl" style={{ fontFamily: 'var(--font-serif)' }}>
                Upload your refusal letter to get started
              </h2>
              <p className="mt-3 text-ink-soft">
                We'll read your refusal, explain what the consular officer found, and help you build your response.
              </p>
              <Link
          ...
          <div className="mx-auto max-w-4xl px-6 py-8">
            ...
            <div className="flex flex-wrap gap-3">
              {page.relatedPages.map(path => {
                const relSlug = path.replace('/visa-refusal/', '');
                if (path === '/visa-refusal') {
                  return (
                    <Link key={path} to="/visa-refusal" className="rounded-full border border-rule px-4 py-2 text-sm hover:border-stamp">
                      Visa Refusal Hub
                    </Link>
                  );
                }
