...
import { createCommunication, getCommunication, type CreateAppealCommunicationInput, type MailType } from "./mailmypdf";
...
export class MailMyPDFProvider implements MailingProvider {
  async createLetter(input: MailingOrderDraft): Promise<{ providerOrderId: string }> {
    ...
    const communicationInput: CreateAppealCommunicationInput = {
      ...
      matter_type: input.matterType ?? "appeal-mail",
      metadata: {
