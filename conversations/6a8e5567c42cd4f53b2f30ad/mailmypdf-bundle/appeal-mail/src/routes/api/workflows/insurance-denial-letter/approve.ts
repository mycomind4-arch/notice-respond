...
export const Route = createFileRoute("/api/workflows/insurance-denial-letter/approve")({
  server: {
    handlers: {
        POST: async ({ request }) => {
    try {
      ...
      if (!appeal.draft?.trim()) return Response.json({ error: "The response draft must be created before approval." }, { status: 409 });
      ...
      if (review.score < 80 || review.issuesRequiringAttention > 2 || review.checks.some((check) => check.status === "fail")) return Response.json({ error: "Response is not ready for approval.", review }, { status: 409 });
      const packet = assemblePacket({ appealId, finalLetter: appeal.draft, evidence, recipient, mailingMethod });
      ...
      if (updateError) throw new Error(`Unable to approve response: ${updateError.message}`);
