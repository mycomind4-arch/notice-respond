...
export const Route = createFileRoute("/api/workflows/insurance-denial-letter/draft")({
  server: {
    ...
    try {
      ...
      return Response.json({ ok: true, appealId: appeal.id, draft: finalDraft, validation, provider: "gemini", draftModel: draftConfig.model, validationModel: validationConfig.model });
    } catch (error) {
