...
async function resolveGemini() {
  ...
  const response = await fetch(`${base.replace(/\/$/, "")}/api/control-plane/ai`, {
    ...
    body: JSON.stringify({ verticalSlug: "appeal-mail", workflowSlug: "dental-insurance-appeal", task: "analysis" }),
  });
...
export const Route = createFileRoute("/api/workflows/dental-insurance-appeal/analyze")({
  server: {
    handlers: {
      POST: async ({ request }) => {
    try {
      ...
      if (!(file instanceof File)) return Response.json({ error: "A dental insurance decision is required." }, { status: 400 });
      ...
      const prompt = [
        ...
        "Analyze the actual dental insurance decision. Extract only supported facts and distinguish insurer statements from records or claimant-provided facts.",
        "Identify the dental procedure or claim, denial rationale, coverage language or limitations cited, claim dates, clinical/dental facts present, records referenced, evidence gaps, appeal instructions, and deadline.",
        "Do not diagnose, invent dental findings, policy language, legal authority, deadlines, or outcomes.",
        "Return strict JSON only.",
        '{"summary":"","decision":"","decisionType":"dental_insurance_denial","issuer":"","referenceNumber":"","decisionDate":"","deadline":"","procedure":"","denialReasons":[],"coverageLanguageCited":[],"dentalFactsSupported":[],"evidenceMentioned":[],"issues":[{"issue":"","whyItMatters":"","evidenceNeeded":[]}],"uncertainties":[],"confidence":"high|medium|low"}'
      ].join("\n\n");
      ...
      const decision = createDecision("claim_denial", {
        ...
        decisionDate: analysis.decisionDate || undefined, decisionTypeLabel: analysis.decision || "Dental insurance denial",
        deadline: analysis.deadline ? { date: analysis.deadline, type: "appeal", source: "extracted" } : undefined,
        facts: [...(analysis.dentalFactsSupported || []), ...(analysis.coverageLanguageCited || [])].map((value: string, index: number) => ({ id: `${index}-${crypto.randomUUID()}`, label: `Fact ${index + 1}`, value, source: "extracted", confidence: 0.8 })),
        reasons: (analysis.denialReasons || []).map((text: string, index: number) => ({ id: `${index}-${crypto.randomUUID()}`, text, confidence: 0.9 })),
        issues: (analysis.issues || []).map((item: any, index: number) => ({ id: `${index}-${crypto.randomUUID()}`, description: item.issue || "Dental insurance issue", type: "factual_dispute", severity: "medium", sourceExcerpt: item.whyItMatters })),
      ...
      const grounds = (analysis.issues || []).map((item: any, index: number) => createGround("factual_error", {
        id: `ground-${index}-${crypto.randomUUID()}`,
        claim: item.issue || "Review the dental insurance finding",
        source: item.whyItMatters || "Identified by document analysis",
        confidence: 0.65,
      ...
      evidence.unshift(createEvidence("document", "Original dental insurance decision", { documentId: document.id, documentFilename: document.filename, uploadedAt: new Date().toISOString() }));
      if (evidence.length && grounds.length) grounds[0].supportingEvidenceIds = evidence.map((item) => item.id);
      const appeal = createAppeal("dental-insurance-appeal", decision);
      ...
      const message = error instanceof Error ? error.message : "Unable to analyze dental insurance decision.";
