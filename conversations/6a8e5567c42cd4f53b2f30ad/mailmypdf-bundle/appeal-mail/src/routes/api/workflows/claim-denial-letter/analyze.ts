...
async function resolveGemini() {
  ...
  const response = await fetch(`${base.replace(/\/$/, "")}/api/control-plane/ai`, {
    ...
    body: JSON.stringify({ verticalSlug: "appeal-mail", workflowSlug: "claim-denial-letter", task: "analysis" }),
  });
...
export const Route = createFileRoute("/api/workflows/claim-denial-letter/analyze")({
  server: {
    handlers: {
      POST: async ({ request }) => {
    try {
      ...
      if (!(file instanceof File)) return Response.json({ error: "A claim denial letter is required." }, { status: 400 });
      if (file.size === 0) return Response.json({ error: "The document is empty." }, { status: 400 });
      ...
      const prompt = [
        ...
        "Analyze the actual claim denial letter, not a hypothetical claim.",
        "Extract what was denied, the exact stated reasons, cited terms or provisions, response instructions, deadlines, disputed facts, and evidence needed.",
        "Do not invent law, policy language, facts, dates, amounts, or outcomes.",
        "Return strict JSON only.",
        '{"summary":"","decision":"","decisionType":"claim_denial_letter","issuer":"","referenceNumber":"","decisionDate":"","deadline":"","reasons":[],"keyFacts":[],"issues":[{"issue":"","whyItMatters":"","evidenceNeeded":[]}],"evidenceMentioned":[],"uncertainties":[],"confidence":"high|medium|low"}',
      ].join("\n\n");
      ...
      const decision = createDecision("claim_denial", {
        ...
        decisionTypeLabel: analysis.decision || "Claim denial letter",
      ...
      const message = error instanceof Error ? error.message : "Unable to analyze claim denial letter.";
