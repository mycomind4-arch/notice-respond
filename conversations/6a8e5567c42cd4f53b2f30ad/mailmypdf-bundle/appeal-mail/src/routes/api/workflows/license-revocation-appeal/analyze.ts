...
function mediaType(file: File): "application/pdf" | "image/png" | "image/jpeg" {
  ...
  throw new Error("Please upload a PDF, PNG, or JPEG revocation decision.");
}
async function resolveGemini() {
  ...
  const response = await fetch(`${base.replace(/\/$/, "")}/api/control-plane/ai`, { method: "POST", headers: { "content-type": "application/json", authorization: `Bearer ${token}` }, body: JSON.stringify({ verticalSlug: "appeal-mail", workflowSlug: "license-revocation-appeal", task: "analysis" }) });
...
export const Route = createFileRoute("/api/workflows/license-revocation-appeal/analyze")({server:{handlers:{POST:async ({ request }) => {
  try {
    ...
    if (!(file instanceof File)) return Response.json({ error: "A license revocation decision is required." }, { status: 400 });
    ...
    evidence.unshift(createEvidence("document", "Original license revocation decision", { documentId: document.id, documentFilename: document.filename, uploadedAt: new Date().toISOString() }));
