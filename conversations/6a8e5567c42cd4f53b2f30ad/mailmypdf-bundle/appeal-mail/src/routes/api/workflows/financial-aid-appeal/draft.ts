...
async function resolveGemini(task: "draft" | "validation") {
  ...
  const response = await fetch(`${base.replace(/\/$/, "")}/api/control-plane/ai`, { method: "POST", headers: { "content-type": "application/json", authorization: `Bearer ${token}` }, body: JSON.stringify({ verticalSlug: "appeal-mail", workflowSlug: "financial-aid-appeal", task }) });
...
export const Route = createFileRoute("/api/workflows/financial-aid-appeal/draft")({
  server: {
    ...
    try {
      ...
      if (appeal.workflow_id !== "financial-aid-appeal") return Response.json({ error: "Appeal workflow mismatch." }, { status: 409 });
      ...
      const draft = input.draftOverride?.trim() || await callGemini(draftConfig, [`Create a response for the workflow: ${workflow.title}.`, workflow.workflowPrompt, `Focus on: ${workflow.focusAreas.join(", ")}.`, "Use only supplied facts. Do not invent school policy, FAFSA facts, academic records, financial figures, deadlines, legal claims, or approval outcomes.", "Write a professional financial-aid appeal that a student/parent can review and edit. Include a concise request and a practical supporting-evidence list.", `CASE ANALYSIS:\n${JSON.stringify(analysis)}`].join("\n\n"));
      const validation = await callGemini(validationConfig, [`Audit this financial-aid appeal draft for ${workflow.title}.`, "Return strict JSON with valid, issues, unsupportedClaims, missingEvidence, suggestions.", "Flag unsupported facts, invented institutional policy, missing evidence, contradictions, deadline issues, and overconfident outcome claims.", `CASE ANALYSIS:\n${JSON.stringify(analysis)}`, `DRAFT:\n${draft}`].join("\n\n"));
