...
export const Route = createFileRoute("/api/workflows/dental-insurance-appeal/approve")({
  server: {
    handlers: {
        POST: async ({ request }) => {
    try {
      ...
      const appealId = input.appealId?.trim(); const recipient = input.recipient;
      ...
      const packet = assemblePacket({ appealId, finalLetter: appeal.draft, evidence, recipient: { name: recipient.name, address1: recipient.address1, address2: recipient.address2, city: recipient.city, state: recipient.state, zip: recipient.zip }, mailingMethod: input.mailingMethod });
      ...
      const message = error instanceof Error ? error.message : "Unable to approve dental insurance appeal.";
