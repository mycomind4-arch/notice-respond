...
async function resolveGemini(task: "draft" | "validation") {
  ...
  const response = await fetch(`${base.replace(/\/$/, "")}/api/control-plane/ai`, { method: "POST", headers: { "content-type": "application/json", authorization: `Bearer ${token}` }, body: JSON.stringify({ verticalSlug: "appeal-mail", workflowSlug: "ssi-denial", task }) });
...
export const Route = createFileRoute("/api/workflows/ssi-denial/draft")({server:{handlers:{ POST: async ({ request }) => {
  try {
    ...
    if (error || !appeal) return Response.json({ error: "Appeal case not found." }, { status: 404 }); if (appeal.user_id !== user.id) return Response.json({ error: "You do not own this appeal case." }, { status: 403 }); if (appeal.workflow_id !== "ssi-denial") return Response.json({ error: "Appeal workflow mismatch." }, { status: 409 });
    const workflow = getWorkflow("ssi-denial"); const draftConfig = await resolveGemini("draft"); const validationConfig = await resolveGemini("validation"); const analysis = input.analysis || appeal.decision;
    const draft = input.draftOverride?.trim() || await callGemini(draftConfig, [`Create a response for the workflow: ${workflow.title}.`, workflow.workflowPrompt, `Focus on: ${workflow.focusAreas.join(", ")}.`, "Use only supplied facts. Do not invent SSA policy, legal authority, medical facts, dates, deadlines, or eligibility facts.", "Write a professional SSI response/appeal that a human can review and edit.", `CASE ANALYSIS:\n${JSON.stringify(analysis)}`].join("\n\n"));
    const validation = await callGemini(validationConfig, [`Audit this SSI response for: ${workflow.title}.`, "Return JSON with valid, issues, unsupportedClaims, missingEvidence, suggestions.", "Flag fabricated SSA/legal authority, unsupported medical/financial facts, missing evidence, contradictions, deadline issues, and factual uncertainty.", `CASE ANALYSIS:\n${JSON.stringify(analysis)}`, `DRAFT:\n${draft}`].join("\n\n"));
    const draftValidation = validateAppealDraft(draft, appeal.decision || ({} as any), Array.isArray(appeal.grounds) ? appeal.grounds : [], Array.isArray(appeal.evidence) ? appeal.evidence : []); const blockingFindings = draftValidation.findings.filter((f) => (f.severity === "block" || f.severity === "error") && !f.passed); if (blockingFindings.length > 0) return Response.json({ error: "Draft failed validation.", draftValidation, blockingFindings }, { status: 422 }); const currentVersion = appeal.version ?? 1; const { error: updateError } = await supabase.from("appeals").update({ draft: `${draft}\n\nSincerely,\n[Your Name]`, status: "in_progress", version: currentVersion + 1, updated_at: new Date().toISOString() }).eq("id", appeal.id).eq("user_id", user.id).eq("version", currentVersion); if (updateError) throw new Error(`Unable to persist draft: ${updateError.message}`);
