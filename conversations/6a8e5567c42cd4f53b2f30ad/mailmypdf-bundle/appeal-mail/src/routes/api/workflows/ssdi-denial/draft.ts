...
export const Route = createFileRoute("/api/workflows/ssdi-denial/draft")({server:{handlers:{ POST: async ({ request }) => { try {
  ...
  const workflow = getWorkflow("ssdi-denial"); const draftConfig = await resolveGemini("draft"); const validationConfig = await resolveGemini("validation"); const analysis = input.analysis || appeal.decision;
  const draft = input.draftOverride?.trim() || await callGemini(draftConfig, [`Create a response for the workflow: ${workflow.title}.`, workflow.workflowPrompt, `Focus on: ${workflow.focusAreas.join(", ")}.`, "Use only supplied facts. Do not invent medical diagnoses, functional limitations, law, SSA policy, dates, deadlines, or outcomes.", "Write a professional SSDI appeal response a human can review and edit. Clearly distinguish user-provided facts from uncertain inferences.", `CASE ANALYSIS:\n${JSON.stringify(analysis)}`].join("\n\n"));
