...
async function resolveGemini(task: "draft" | "validation") {
  ...
  const response = await fetch(`${base.replace(/\/$/, "")}/api/control-plane/ai`, { method: "POST", headers: { "content-type": "application/json", authorization: `Bearer ${token}` }, body: JSON.stringify({ verticalSlug: "appeal-mail", workflowSlug: "dental-insurance-appeal", task }) });
...
export const Route = createFileRoute("/api/workflows/dental-insurance-appeal/draft")({
  server: {
    handlers: {
        POST: async ({ request }) => {
    try {
      ...
      if (appeal.workflow_id !== "dental-insurance-appeal") return Response.json({ error: "Appeal workflow mismatch." }, { status: 409 });
      ...
      const draft = input.draftOverride?.trim() || await callGemini(draftConfig, [
        ...
        "Use only supplied facts. Do not invent dental findings, coverage language, legal authority, dates, deadlines, or outcomes.",
        "Write a professional dental insurance appeal that a human can review and edit.", `CASE ANALYSIS:\n${JSON.stringify(analysis)}`,
      ].join("\n\n"));
      const validation = await callGemini(validationConfig, [
        ...
        "Flag unsupported dental/clinical claims, invented policy or legal authority, missing documentation, contradictions, deadline problems, and uncertainty.",
      ...
      const { error: updateError } = await supabase.from("appeals").update({ draft: persisted, status: "in_progress", version: currentVersion + 1, updated_at: new Date().toISOString() }).eq("id", appeal.id).eq("user_id", user.id).eq("version", currentVersion);
      ...
      const message = error instanceof Error ? error.message : "Unable to create dental insurance appeal response.";
