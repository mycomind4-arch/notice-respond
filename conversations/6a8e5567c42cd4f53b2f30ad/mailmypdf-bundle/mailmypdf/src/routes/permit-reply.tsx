...
import { permitReplyProduct, type PermitReplyInput, type PermitReplyAnalysis } from "@/products/permit-reply";
...
export const Route = createFileRoute("/permit-reply")({
  head: () => ({
    meta: [
      { title: "PermitReply — Respond to Permit Notices | MailMyPDF" },
      { name: "description", content: "Draft a response to permit notices and applications. Mail it certified with proof of delivery." },
      { property: "og:title", content: "PermitReply — Respond to Permit Notices" },
      { property: "og:description", content: "Draft a response to permit notices. Certified mail with proof of delivery." },
    ],
  }),
  component: PermitReplyPage,
});
...
const emptyInput: PermitReplyInput = {
  noticeType: "", agencyName: "", agencyAddress: "", permitNumber: "",
  noticeDate: "", responseDeadline: "", noticeSummary: "", applicantPosition: "",
  evidenceItems: "", propertyAddress: "", projectDescription: "",
  applicantName: "", applicantAddress: "", applicantEmail: "", applicantPhone: "",
  additionalNotes: "",
  documentText: "",
};
...
function PermitReplyPage() {
  const createLetterOrderFn = useServerFn(createLetterOrder);
  const createCheckoutFn = useServerFn(createCheckoutForOrder);

  const [step, setStep] = useState(0);
  const [input, setInput] = useState<PermitReplyInput>(emptyInput);
  const [analysis, setAnalysis] = useState<PermitReplyAnalysis | null>(null);
  ...
  const set = (key: keyof PermitReplyInput, value: string) =>
    setInput((prev) => ({ ...prev, [key]: value }));

  async function analyzeAndDraft() {
    if (!input.noticeType || !input.agencyName || !input.noticeSummary.trim() || !input.applicantPosition.trim() || !input.applicantName) {
      setError("Fill in the notice type, agency name, notice summary, your position, and your name.");
      return;
    }
    setBusy(true);
    setError("");
    try {
      const analysisRes = await fetch("/api/v1/permit-reply/analyze", {
        method: "POST",
      ...
      setAnalysis(analysisPayload.analysis as PermitReplyAnalysis);

      const draftRes = await fetch("/api/v1/permit-reply/draft", {
        method: "POST",
        headers: { "content-type": "application/json" },
        body: JSON.stringify({ ...input, documentText }),
      });
  ...
  return (
    <div className="min-h-screen"><SiteHeader /><main className="min-h-screen bg-[#f6f4ef] text-[#17201d]">
      <section className="mx-auto grid max-w-6xl gap-10 px-6 py-12 lg:grid-cols-[.72fr_1.28fr]">
        <aside>
          <p className="text-xs font-semibold uppercase tracking-[.2em]" style={{ color: ACCENT }}>Permit response</p>
          <h1 className="mt-4 font-serif text-5xl leading-none md:text-6xl">
            Respond to permit notices.
            <span className="block" style={{ color: ACCENT }}>On the record.</span>
          </h1>
          <p className="mt-6 max-w-md text-lg leading-8 text-[#17201d]/65">
            {permitReplyProduct.description}
          </p>
          ...
          {analysis?.deadlineInfo && (
            <div className={`mt-8 rounded-xl border p-4 ${analysis.deadlinePassed ? "border-red-200 bg-red-50" : "border-[#1e40af]/20"}`} style={!analysis.deadlinePassed ? { background: ACCENT_LIGHT } : {}}>
              ...
              {!analysis.deadlinePassed && <p className="mt-1 text-xs text-[#17201d]/55">Mail your response as soon as possible.</p>}
            </div>
          )}

          {analysis?.processOverview && (
            <div className="mt-4 rounded-xl border border-[#17201d]/10 bg-white p-4">
              <p className="text-xs font-semibold uppercase tracking-wide" style={{ color: ACCENT }}>The process</p>
              <p className="mt-1 text-xs text-[#17201d]/65">{analysis.processOverview}</p>
            </div>
          )}
              ...
              <div className="space-y-6">
                <div>
                  <label className="block">
                    <span className="text-sm font-medium">What type of permit notice did you receive?</span>
                    <select value={input.noticeType} onChange={(e) => set("noticeType", e.target.value)} className="mt-2 w-full rounded-lg border border-[#17201d]/15 bg-white px-3 py-3 text-sm">
                      <option value="">Select a notice type…</option>
                      {permitReplyProduct.noticeTypes.map((t) => <option key={t} value={t}>{t}</option>)}
                    </select>
                  </label>
                </div>
                ...
                <div className="grid gap-4 sm:grid-cols-2">
                  <label className="block">
                    ...
                    <input type="text" value={input.agencyName} onChange={(e) => set("agencyName", e.target.value)} placeholder="e.g. City of Springfield Building Dept." className="mt-2 w-full rounded-lg border border-[#17201d]/15 bg-white px-3 py-3 text-sm" />
                  </label>
                  <label className="block">
                ...
                <div className="grid gap-4 sm:grid-cols-2">
                  <label className="block">
                    <span className="text-sm font-medium">Permit number (if any)</span>
                    <input type="text" value={input.permitNumber} onChange={(e) => set("permitNumber", e.target.value)} placeholder="e.g. BLD-2024-1234" className="mt-2 w-full rounded-lg border border-[#17201d]/15 bg-white px-3 py-3 text-sm" />
                  </label>
                  <label className="block">
                    <span className="text-sm font-medium">Date of notice</span>
                    <input type="date" value={input.noticeDate} onChange={(e) => set("noticeDate", e.target.value)} className="mt-2 w-full rounded-lg border border-[#17201d]/15 bg-white px-3 py-3 text-sm" />
                  </label>
                </div>

                <label className="block">
                  <span className="text-sm font-medium">Response deadline (if any)</span>
                  <input type="date" value={input.responseDeadline} onChange={(e) => set("responseDeadline", e.target.value)} className="mt-2 w-full rounded-lg border border-[#17201d]/15 bg-white px-3 py-3 text-sm" />
                </label>

                <label className="block">
                  <span className="text-sm font-medium">What does the notice say?</span>
                  <textarea value={input.noticeSummary} onChange={(e) => set("noticeSummary", e.target.value)} placeholder="Summarize the notice — what is the agency requiring, denying, or notifying you about?" rows={4} className="mt-2 w-full resize-y rounded-lg border border-[#17201d]/15 bg-white px-3 py-3 text-sm" />
                </label>

                <label className="block">
                  <span className="text-sm font-medium">Your position / response</span>
                  <textarea value={input.applicantPosition} onChange={(e) => set("applicantPosition", e.target.value)} placeholder="What's your side? Do you agree, disagree, or want to request modifications? What do you want to happen?" rows={4} className="mt-2 w-full resize-y rounded-lg border border-[#17201d]/15 bg-white px-3 py-3 text-sm" />
                </label>

                <label className="block">
                  <span className="text-sm font-medium">Evidence / documents you're including</span>
                  <textarea value={input.evidenceItems} onChange={(e) => set("evidenceItems", e.target.value)} placeholder="List what you're enclosing — plans, photos, engineer reports, prior correspondence…" rows={3} className="mt-2 w-full resize-y rounded-lg border border-[#17201d]/15 bg-white px-3 py-3 text-sm" />
                </label>

                <div className="grid gap-4 sm:grid-cols-2">
                  <label className="block">
                    <span className="text-sm font-medium">Property address</span>
                    <input type="text" value={input.propertyAddress} onChange={(e) => set("propertyAddress", e.target.value)} placeholder="Street, city, state, ZIP" className="mt-2 w-full rounded-lg border border-[#17201d]/15 bg-white px-3 py-3 text-sm" />
                  </label>
                  <label className="block">
                    <span className="text-sm font-medium">Project description</span>
                    <input type="text" value={input.projectDescription} onChange={(e) => set("projectDescription", e.target.value)} placeholder="e.g. Two-story addition, deck, fence…" className="mt-2 w-full rounded-lg border border-[#17201d]/15 bg-white px-3 py-3 text-sm" />
                  </label>
                </div>
                ...
                <div className="grid gap-4 sm:grid-cols-2">
                  <label className="block">
                    ...
                    <input type="text" value={input.applicantName} onChange={(e) => set("applicantName", e.target.value)} placeholder="Full name" className="mt-2 w-full rounded-lg border border-[#17201d]/15 bg-white px-3 py-3 text-sm" />
                  </label>
                  <label className="block">
                    <span className="text-sm font-medium">Your email</span>
                    <input type="email" value={input.applicantEmail} onChange={(e) => set("applicantEmail", e.target.value)} placeholder="you@example.com" className="mt-2 w-full rounded-lg border border-[#17201d]/15 bg-white px-3 py-3 text-sm" />
                  </label>
                </div>
                ...
                <label className="block">
                  ...
                  <textarea value={input.additionalNotes} onChange={(e) => set("additionalNotes", e.target.value)} placeholder="Additional context, prior permits, special circumstances…" rows={2} className="mt-2 w-full resize-y rounded-lg border border-[#17201d]/15 bg-white px-3 py-3 text-sm" />
                </label>
                ...
                {analysis && (
                  <div className="space-y-3">
                    ...
                    {analysis.tips.length > 0 && (
                      <div className="rounded-lg border border-[#1e40af]/20 p-4" style={{ background: ACCENT_LIGHT }}>
              ...
              <div className="space-y-6">
                ...
                <div>
                  ...
                  <div className="grid gap-3">
                    ...
                    <input type="text" placeholder="Suite (optional)" value={sender.line2} onChange={(e) => setSender({ ...sender, line2: e.target.value })} className="rounded-lg border border-[#17201d]/15 bg-white px-3 py-3 text-sm" />
                ...
                <div>
                  ...
                  <div className="grid gap-2 sm:grid-cols-3">
                    {(Object.keys(MAIL_CLASS_LABELS) as MailClass[]).map((mc) => (
                      <button key={mc} onClick={() => setMailClass(mc)} className={`rounded-lg border px-3 py-2.5 text-left text-sm transition ${mailClass === mc ? "border-[#1e40af] bg-[#eff6ff]" : "border-[#17201d]/15 hover:border-[#17201d]/30"}`}>
