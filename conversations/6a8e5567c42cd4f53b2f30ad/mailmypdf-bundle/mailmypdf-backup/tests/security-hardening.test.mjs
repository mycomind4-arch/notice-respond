...
describe("Security Hardening — Input Sanitization", () => {
  ...
  it("orders.functions.ts imports and uses sanitizers", async () => {
    const orders = await source("src/lib/orders.functions.ts");
    assert.match(orders, /import.*sanitize.*from.*sanitize/);
    assert.match(orders, /sanitizeAddress/);
    assert.match(orders, /sanitizeEmail/);
    assert.match(orders, /sanitizeFileName/);
  });
});
