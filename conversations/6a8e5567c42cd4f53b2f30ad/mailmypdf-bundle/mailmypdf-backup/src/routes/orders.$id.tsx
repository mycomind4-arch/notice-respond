      ...
      <div className="min-h-screen">
        ...
        <main className="mx-auto max-w-2xl px-6 py-24 text-center">
          ...
          <p className="mt-3 text-muted-foreground">
            Open the exact link from your confirmation email — it includes a token that unlocks this page.
          </p>
        </main>
        <SiteFooter />
      </div>
      ...
      <div className="mt-8 grid gap-6 md:grid-cols-[1.4fr_1fr]">
        <div className="envelope-card envelope-card-notch p-8">
          ...
          <div className="mt-6 grid gap-6 sm:grid-cols-2">
            ...
            <div>
              <div className="text-[10px] uppercase tracking-[0.25em] text-muted-foreground">Mailed to</div>
            ...
            <div>
              ...
              <div className="mt-1 font-mono text-sm" suppressHydrationWarning>{createdAt}</div>
            </div>
          </div>
          ...
          <ol className="mt-4 space-y-4">
            {events.map((e, i) => (
              <li key={i} className="relative pl-6">
                ...
                <div className="font-mono text-[11px] uppercase tracking-widest text-muted-foreground" suppressHydrationWarning>
            ...
            {status !== "mailed" && status !== "delivered" && status !== "failed" && (
              <li className="relative pl-6 opacity-50">
                ...
                <div className="font-mono text-[11px] uppercase tracking-widest text-muted-foreground">Pending</div>
              </li>
            )}
          </ol>
        </div>
...
function StatusBadge({ status }: { status: StatusKey }) {
  return (
    <div className="inline-flex items-center gap-3 rounded-full border border-stamp/40 bg-stamp/10 px-4 py-1.5">
      ...
      <span className="font-mono text-xs uppercase tracking-widest text-stamp">{STATUS_LABEL[status]}</span>
    </div>
  );
}
