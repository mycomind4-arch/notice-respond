...
export function generateMetadata() {
  return {
    title: `Debt Defense Workflows — All | Debt Defense`,
    description: `Browse all Debt Defense workflows organized by category. Upload the decision or notice, organize evidence, draft a response, review, and mail with proof.`,
    openGraph: { title: `Debt Defense Workflows`, description: 'Problem-specific workflows with document upload, evidence organization, response drafting, review, and mailing proof.', type: 'website', siteName: 'Debt Defense', url: SITE_ORIGIN + '/workflows' },
    other: { 'application/ld+json': JSON.stringify({ '@context': 'https://schema.org', '@type': 'ItemList', name: 'Debt Defense Workflows', itemListElement: WORKFLOWS.map((w, i) => ({ '@type': 'ListItem', position: i + 1, name: w.name, url: SITE_ORIGIN + '/workflows/' + w.id })) }) },
  }
}
                ...
                {workflows.map(w => (
                  <Link key={w.id} href={`/workflows/${w.id}`} className="card" style={{padding:22,display:'block'}}>
                    ...
                    <div style={{display:'flex',gap:14,marginTop:14,paddingTop:14,borderTop:'1px solid rgba(255,255,255,.06)'}}>
                      <span style={{fontSize:12,color:'#f87171',fontWeight:600}}>{w.primaryKeyword}</span>
