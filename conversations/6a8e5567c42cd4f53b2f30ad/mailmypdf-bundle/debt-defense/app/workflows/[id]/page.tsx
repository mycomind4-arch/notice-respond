...
import { STAGES } from '@/domain/debt-workflows'
import { workflowMap, WORKFLOWS, type WorkflowId } from '@/domain/debt-workflows'

const SITE_ORIGIN = 'https://debt-defense.pages.dev'
...
export async function generateMetadata({ params }: { params: Promise<{ id: string }> }) {
  ...
  if (!workflow) return { title: 'Debt Defense Workflow' }
  return {
    title: `${workflow.name} | Debt Defense`,
    description: workflow.description,
    keywords: [workflow.primaryKeyword, ...workflow.supportingKeywords],
    alternates: { canonical: `/workflows/${workflow.id}` },
    openGraph: { title: `${workflow.name} | Debt Defense`, description: workflow.description, type: 'article', siteName: 'Debt Defense', url: `${SITE_ORIGIN}/workflows/${workflow.id}` },
    twitter: { card: 'summary', title: `${workflow.name} | Debt Defense`, description: workflow.description },
      ...
      <section style={{paddingTop:32,paddingBottom:40}}>
        <div className="container">
          ...
          <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(160px,1fr))',gap:14,maxWidth:680}}>
            ...
            <div className="stat-card"><div className="muted" style={{fontSize:12}}>Primary keyword</div><div style={{fontSize:15,fontWeight:600,marginTop:6,color:'#f87171'}}>{workflow.primaryKeyword}</div></div>
          </div>
        </div>
      </section>
      ...
      <section className="section-tight" style={{borderTop:'1px solid rgba(255,255,255,.08)'}}>
        <div className="container">
          ...
          <div style={{display:'flex',flexWrap:'wrap',gap:10}}>
            <span style={{padding:'8px 16px',borderRadius:10,background:'rgba(f87171,.1)',color:'#f87171',fontSize:13,fontWeight:600}}>{workflow.primaryKeyword} ({workflow.monthlySearches.toLocaleString()}/mo)</span>
