...
import { WORKFLOWS, type WorkflowFamily } from '@/domain/debt-workflows'

export const metadata = {
  title: 'Debt Defense | Decision, Denial & Appeal Workflows',
  description: 'Debt Defense — focused workflows for responding to government decisions, denials, and notices. Upload documents, organize evidence, draft a response, review, and mail with proof.',
}
    ...
    <main>
      ...
      <section className="hero-gradient" style={{borderBottom:'1px solid rgba(255,255,255,.08)'}}>
        <div className="container" style={{paddingTop:88,paddingBottom:88}}>
          <div className="eyebrow">DEBT DEFENSE · {totalWorkflows} WORKFLOWS</div>
          <h1 style={{fontSize:'clamp(42px,7vw,80px)',lineHeight:1.02,fontWeight:800,letterSpacing:'-.03em',margin:'20px 0',maxWidth:900}}>
            Find the debt defense workflow that matches your situation.
          </h1>
          ...
          <div style={{display:'flex',gap:32,marginTop:48,flexWrap:'wrap'}}>
            <div><div style={{fontSize:32,fontWeight:800,color:'#f87171'}}>{totalWorkflows}</div><div className="muted" style={{fontSize:13}}>Focused workflows</div></div>
            <div><div style={{fontSize:32,fontWeight:800,color:'#f87171'}}>{families.length}</div><div className="muted" style={{fontSize:13}}>Categories</div></div>
            <div><div style={{fontSize:32,fontWeight:800,color:'#f87171'}}>{(totalSearchVolume/1000).toFixed(0)}K</div><div className="muted" style={{fontSize:13}}>Monthly search volume</div></div>
            <div><div style={{fontSize:32,fontWeight:800,color:'#f87171'}}>8</div><div className="muted" style={{fontSize:13}}>Pipeline stages</div></div>
          </div>
        </div>
      </section>
          ...
          <div style={{display:'grid',gap:16,gridTemplateColumns:'repeat(auto-fit,minmax(240px,1fr))'}}>
            {[
              ...
              <div className="card" style={{padding:24}} key={n}>
                <div style={{fontSize:12,fontWeight:800,color:'#f87171'}}>{n}</div>
              ...
              return (
                <Link key={family} href={`/workflows?family=${encodeURIComponent(family)}`} className="card" style={{padding:28,display:'block'}}>
                  ...
                  <div style={{marginTop:16,display:'flex',flexWrap:'wrap',gap:8}}>
                    ...
                    {workflows.length > 4 && <span style={{fontSize:12,padding:'4px 10px',color:'#f87171'}}>+{workflows.length - 4} more</span>}
                  </div>
                </Link>
              )
            })}
            ...
            {WORKFLOWS.slice().sort((a,b) => b.monthlySearches - a.monthlySearches).slice(0,9).map(w => (
              <Link key={w.id} href={`/workflows/${w.id}`} className="card" style={{padding:24,display:'block'}}>
                ...
                <div style={{display:'flex',gap:16,marginTop:16}}>
                  <span style={{fontSize:12,color:'#f87171'}}>{w.monthlySearches.toLocaleString()}/mo</span>
      ...
      <section id="status" className="section" style={{borderTop:'1px solid rgba(255,255,255,.08)'}}>
        <div className="container">
          <div className="card" style={{padding:36,borderColor:'rgba(251,191,36,.2)',background:'rgba(251,191,36,.03)'}}>
            ...
            <p style={{margin:0,maxWidth:720,fontSize:15,lineHeight:1.7,color:'#94a3b8'}}>These pages are the master information architecture for Debt Defense. We are not presenting unfinished workflows as fully operational. Each workflow will become executable as its product implementation is migrated into this repository.</p>
          </div>
        </div>
      </section>
      ...
      <footer style={{borderTop:'1px solid rgba(255,255,255,.08)',padding:'40px 0'}}>
        <div className="container" style={{display:'flex',justifyContent:'space-between',alignItems:'center',flexWrap:'wrap',gap:16}}>
          <div><div style={{fontWeight:800,fontSize:16}}>Debt Defense</div><div style={{fontSize:13,color:'#94a3b8',marginTop:4}}>{totalWorkflows} workflows · {families.length} categories</div></div>
