/* ═══════════════════════════════════════════════════════════
   SHARED CREDIT DISPUTE DOMAIN LOGIC — generalized extraction
   and draft generation for all three major credit bureaus.
   ...
   This module is parameterized by bureau configuration so each
   workflow only needs to provide bureau-specific details (name,
   mailing address, phone) rather than reimplementing extraction
   and draft logic.
...
// ── Types (shared with transunion-dispute.ts for backward compat) ──
...
export interface CreditDisputeExtraction {
  bureauId: BureauConfig["id"];
  bureauName: string;
  isBureauReport: boolean;
...
function extractConsumerName(text: string): string | null {
  const patterns = [
    /(?:consumer|name|prepared\s+for|report\s+for)\s*:?\s*([A-Z][a-z]+\s+[A-Z][a-z]+(?:\s+[A-Z][a-z]+)?)/i,
  ];
...
function extractDisputedItems(text: string): DisputedItem[] {
  ...
  while ((match = accountPattern.exec(text)) !== null) {
    ...
    items.push({ accountName, accountNumber, errorType, errorDescription, correctInformation: null });
  }
  return items;
}

export function extractCreditDispute(text: string, bureauId: BureauConfig["id"]): CreditDisputeExtraction {
  const config = BUREAU_CONFIGS[bureauId];
  const isBureauReport = config.detectionPattern.test(text);
  ...
  if (isBureauReport) facts.push(createFact("Credit Bureau", config.displayName, "extracted", "high", { extractionMethod: "pattern", sourceExcerpt: text.substring(0, 200) }));
  ...
  for (const item of disputedItems) {
    facts.push(createFact("Disputed: " + (item.accountName ?? "Unknown Account"), item.errorDescription, "extracted", "medium", { extractionMethod: "pattern", sourceExcerpt: text.substring(0, 200) }));
  }

  const warnings: string[] = [];
  if (!isBureauReport) warnings.push("Document may not be a " + config.displayName + " credit report — verify before proceeding.");
  if (!consumerName) warnings.push("Consumer name not found — you will need to provide this manually.");
  if (!reportDate) warnings.push("Report date not found — the bureau must investigate within 30 days of your dispute.");
  ...
  return {
    bureauId,
    bureauName: config.displayName,
    isBureauReport,
    classificationConfidence: isBureauReport ? 0.9 : 0.3,
    consumerName,
    consumerAddress,
    reportDate,
    reportNumber,
