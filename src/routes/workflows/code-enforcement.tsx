      ...
      {step === 4 && (
        <div>
          ...
          {llmAnalysis.llmAnalysis && (
            <button
              onClick={async () => {
                const r = await authFetch("/api/workflows/draft", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ workflowId: "code-enforcement", analysis: llmAnalysis.llmAnalysis, userFacts: facts, userObjective: objective }) });
