...
export const Route = createFileRoute("/workflows/dmv-notice")({
  head: () => createWorkflowHead("dmv-notice"),
  component: dmvnotice,
});
...
function dmvnotice() {
  const definition = workflows["dmv-notice"];
  ...
  function generateDraft() {
    return `Re: Response to Respond to a DMV Notice
      ...
      try {
        if (llmAnalysis.llmAnalysis) {
          const r = await authFetch("/api/workflows/draft", {
            ...
            body: JSON.stringify({ workflowId: "dmv-notice", analysis: llmAnalysis.llmAnalysis, userFacts: facts, userObjective: objective }),
          });
          if (r.ok) {
    ...
    <div className="min-h-screen bg-paper">
      ...
      {step === 0 && (
        <div>
          ...
          <h2 className="mt-4 font-serif text-4xl">Respond to a DMV Notice</h2>
          <p className="mt-3 text-muted-foreground">Organize a DMV notice, identify the response or hearing date, and assemble the records needed for the written response.</p>
          <div className="mt-6 rounded-md border border-rule/70 bg-paper-deep/40 p-4 text-sm text-muted-foreground">
            ...
            <p className="mt-2">{definition?.disclaimer ?? "DMV procedures vary by state. Notice Respond is not a law firm and does not provide legal advice."}</p>
          </div>
          {(() => { const seo = getWorkflowSEO("dmv-notice"); return seo ? <FAQSection faq={seo.faq} /> : null; })()}
        </div>
      )}
      {step === 1 && (
        <div>
          ...
          <div className="mt-6 grid gap-4 sm:grid-cols-2">
            <div><label className="input-label">Agency</label><input className="input-field" value={agency} onChange={(e) => setAgency(e.target.value)} placeholder="DMV" /></div>
      ...
      {step === 4 && (
        <div>
          ...
          {llmAnalysis.llmAnalysis && (
            <button
              onClick={async () => {
                const r = await authFetch("/api/workflows/draft", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ workflowId: "dmv-notice", analysis: llmAnalysis.llmAnalysis, userFacts: facts, userObjective: objective }) });
