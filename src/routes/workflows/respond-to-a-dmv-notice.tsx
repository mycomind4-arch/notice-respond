...
const workflow = { ...NOTICE_WORKFLOWS.find((item) => item.slug === "dmv-notice")!, canonicalPath: "/workflows/analyze" };
export const Route = createFileRoute("/workflows/respond-to-a-dmv-notice")({
  head: () => ({ ...AuthorityWorkflowHead({ workflow }), links: [{ rel: "canonical", href: "/workflows/respond-to-a-dmv-notice" }], scripts: [AuthorityWorkflowStructuredData({ workflow })] }),
