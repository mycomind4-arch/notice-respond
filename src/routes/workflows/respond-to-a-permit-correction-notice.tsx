...
const workflow = { ...NOTICE_WORKFLOWS.find((item) => item.slug === "permit-correction")!, canonicalPath: "/workflows/analyze" };
export const Route = createFileRoute("/workflows/respond-to-a-permit-correction-notice")({
  head: () => ({ ...AuthorityWorkflowHead({ workflow }), links: [{ rel: "canonical", href: "/workflows/respond-to-a-permit-correction-notice" }], scripts: [AuthorityWorkflowStructuredData({ workflow })] }),
