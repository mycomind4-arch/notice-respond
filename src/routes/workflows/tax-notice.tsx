...
export const Route = createFileRoute("/workflows/tax-notice")({
  head: () => createWorkflowHead("tax-notice"),
  component: taxnotice,
});
...
function taxnotice() {
  const definition = workflows["tax-notice"];
  ...
  const llmAnalysis = useCombinedAnalysis("tax-notice");

  function generateDraft() {
    return `Re: Response to Respond to a Tax Notice
...
[Your Name]`;
      ...
      try {
        if (llmAnalysis.llmAnalysis) {
          const r = await authFetch("/api/workflows/draft", {
            ...
            body: JSON.stringify({ workflowId: "tax-notice", analysis: llmAnalysis.llmAnalysis, userFacts: facts, userObjective: objective }),
          });
          if (r.ok) {
  ...
  if (done) return <Success title="Your response has been submitted" href="/workflows/tax-notice" />;
  ...
  return (
    <div className="min-h-screen bg-paper">
      ...
      <main>
        ...
        <section className="relative overflow-hidden border-b border-rule/60">
          ...
          <div className="relative mx-auto max-w-4xl px-4 py-14 sm:px-6 sm:py-20 md:py-28">
            <nav className="flex items-center gap-1.5 text-xs text-muted-foreground" aria-label="Breadcrumb">
              ...
              <span className="text-rule">/</span>
              <span className="text-ink-soft">Tax Authority Notice</span>
            </nav>
            <div className="postmark w-fit mt-6">Tax Authority Notice</div>
            <h1 className="mt-6 font-serif text-4xl leading-[1.1] sm:text-5xl md:text-6xl">
              Respond to your <span className="italic text-stamp">tax notice</span>
            </h1>
            <p className="mt-6 max-w-2xl text-base leading-7 text-ink-soft sm:text-lg">
              You received a notice from a state or local tax authority. Upload it, extract the key details, and prepare a documented response with your facts and supporting evidence.
            </p>
            ...
            <div className="mt-12 grid grid-cols-2 gap-px overflow-hidden rounded-lg border border-rule/60 bg-rule/60 sm:grid-cols-4">
              <KeyFact label="Notice type" value="Tax Authority" />
              <KeyFact label="Jurisdiction" value="State or local" />
        ...
        <section className="border-b border-rule/60">
          <div className="mx-auto max-w-3xl px-4 py-12 sm:px-6 sm:py-16">
            ...
            <h2 className="mt-3 font-serif text-3xl leading-tight">What is a tax authority notice?</h2>
            <div className="mt-6 space-y-4 text-base leading-7 text-ink-soft">
              <p>A tax authority notice is an official communication from a state or local tax agency — such as a state Department of Revenue, franchise tax board, or county assessor. It may notify you of a balance due, a filing requirement, an audit, a correction to your return, or a lien.</p>
              <p>These notices have strict response deadlines, typically 30 days. Missing the deadline can result in automatic assessments, additional penalties, or collection actions. Each jurisdiction has its own appeal process, but all require a written response with supporting documentation.</p>
              <p>Responding early gives you the opportunity to correct errors, request a payment plan, or contest the assessment before enforcement begins. Most tax agencies are willing to work with taxpayers who initiate contact promptly.</p>
            </div>
            <div className="mt-8 rounded-lg border border-rule/60 bg-paper-deep/30 p-5">
              ...
              <ul className="mt-3 grid gap-2 sm:grid-cols-2">
                <li className="flex items-center gap-2 text-sm text-ink-soft"><span className="text-stamp">▸</span>Agency name and contact info</li>
                <li className="flex items-center gap-2 text-sm text-ink-soft"><span className="text-stamp">▸</span>Notice/reference number</li>
                <li className="flex items-center gap-2 text-sm text-ink-soft"><span className="text-stamp">▸</span>Tax period and amount due</li>
                <li className="flex items-center gap-2 text-sm text-ink-soft"><span className="text-stamp">▸</span>Penalties and interest</li>
        ...
        <section className="border-b border-rule/60 bg-paper-deep/20">
          <div className="mx-auto max-w-4xl px-4 py-12 sm:px-6 sm:py-16">
            ...
            <div className="mt-8 grid gap-6 md:grid-cols-3">
              <ProcessStep number="01" title="Upload & analyze" text="Upload the notice PDF or paste the text. AI extracts the agency, notice number, deadline, and amount — then identifies what needs your attention." />
              <ProcessStep number="02" title="Review & draft" text="Add your facts and objective. Generate a professional response letter addressing the specific notice type and your desired outcome." />
              <ProcessStep number="03" title="Mail with proof" text="Approve the exact draft. Certified mail provides tracking and delivery confirmation — your record of timely response." />
            </div>
          </div>
        </section>
        ...
        <section ref={workflowRef} className="border-b border-rule/60" style={{ scrollMarginTop: "80px" }}>
          <div className="mx-auto max-w-5xl px-4 py-12 sm:px-6">
            {workflowStarted ? (
              <WorkflowShell title="Respond to a Tax Notice" steps={STEPS} step={step} setStep={setStep} canContinue={canContinue()} onNext={next} onBack={() => setStep((s) => Math.max(s - 1, 0))}>
      {step === 0 && (
        <div>
          <div className="postmark w-fit">1 · Start</div>
          <h2 className="mt-4 font-serif text-4xl">Respond to a Tax Notice</h2>
          <p className="mt-3 text-muted-foreground">Create a structured response to a tax authority notice without losing the notice deadline, reference number, or supporting evidence.</p>
          <div className="mt-6 rounded-md border border-rule/70 bg-paper-deep/40 p-4 text-sm text-muted-foreground">
            ...
            <p className="mt-2">{definition?.disclaimer ?? "Tax notices vary by jurisdiction. Notice Respond is not a tax preparer or financial advisor and does not provide tax advice."}</p>
          </div>
          {(() => { const seo = getWorkflowSEO("tax-notice"); return seo ? <FAQSection faq={seo.faq} /> : null; })()}
        </div>
      )}
      {step === 1 && (
        <div>
          ...
          <div className="mt-6 grid gap-4 sm:grid-cols-2">
            <div><label className="input-label">Agency</label><input className="input-field" value={agency} onChange={(e) => setAgency(e.target.value)} placeholder="Tax Authority" /></div>
      ...
      {step === 4 && (
        <div>
          ...
          {llmAnalysis.llmAnalysis && (
            <button
              onClick={async () => {
                const r = await authFetch("/api/workflows/draft", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ workflowId: "tax-notice", analysis: llmAnalysis.llmAnalysis, userFacts: facts, userObjective: objective }) });
      ...
      {step === 7 && (<div><div className="postmark w-fit">8 · Recipient</div><h2 className="mt-4 font-serif text-3xl">Where should we send it?</h2><p className="mt-3 text-muted-foreground">Enter the agency's mailing address.</p><RecipientForm recipient={recipient} setRecipient={setRecipient} orgPlaceholder={agency || "Tax Authority"} /></div>)}
        ...
        <section className="border-b border-rule/60">
          <div className="mx-auto max-w-3xl px-4 py-12 sm:px-6">
            ...
            <div className="mt-6 grid gap-4 sm:grid-cols-3">
              <RelatedCard href="/workflows/irs-notice" title="IRS Notice" desc="Any IRS notice not covered by a specific workflow" />
              <RelatedCard href="/workflows/cp14-response" title="CP14 - Balance Due" desc="First IRS collection notice" />
              <RelatedCard href="/workflows/code-enforcement" title="Code Enforcement" desc="Respond to municipal code violations" />
            </div>
