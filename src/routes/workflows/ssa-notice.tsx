...
export const Route = createFileRoute("/workflows/ssa-notice")({
  head: () => createWorkflowHead("ssa-notice"),
  component: ssanotice,
});
...
function ssanotice() {
  const definition = workflows["ssa-notice"];
  ...
  const llmAnalysis = useCombinedAnalysis("ssa-notice");

  function generateDraft() {
    return `Re: Response to Respond to an SSA Notice
...
[Your Name]`;
      ...
      try {
        if (llmAnalysis.llmAnalysis) {
          const r = await authFetch("/api/workflows/draft", {
            ...
            body: JSON.stringify({ workflowId: "ssa-notice", analysis: llmAnalysis.llmAnalysis, userFacts: facts, userObjective: objective }),
          });
          if (r.ok) {
  ...
  if (done) return <Success title="Your response has been submitted" href="/workflows/ssa-notice" />;
  ...
  return (
    <div className="min-h-screen bg-paper">
      ...
      <main>
        ...
        <section className="relative overflow-hidden border-b border-rule/60">
          ...
          <div className="relative mx-auto max-w-4xl px-4 py-14 sm:px-6 sm:py-20 md:py-28">
            <nav className="flex items-center gap-1.5 text-xs text-muted-foreground" aria-label="Breadcrumb">
              ...
              <span className="text-rule">/</span>
              <span className="text-ink-soft">Social Security</span>
            </nav>
            <div className="postmark w-fit mt-6">Social Security</div>
            <h1 className="mt-6 font-serif text-4xl leading-[1.1] sm:text-5xl md:text-6xl">
              Respond to your <span className="italic text-stamp">SSA notice</span>
            </h1>
            <p className="mt-6 max-w-2xl text-base leading-7 text-ink-soft sm:text-lg">
              The Social Security Administration sent a notice about your benefits, overpayment, or eligibility. Upload it, understand the determination, and prepare a documented response.
            </p>
            ...
            <div className="mt-12 grid grid-cols-2 gap-px overflow-hidden rounded-lg border border-rule/60 bg-rule/60 sm:grid-cols-4">
              <KeyFact label="Notice type" value="SSA" />
        ...
        <section className="border-b border-rule/60">
          <div className="mx-auto max-w-3xl px-4 py-12 sm:px-6 sm:py-16">
            ...
            <h2 className="mt-3 font-serif text-3xl leading-tight">What is an SSA notice?</h2>
            <div className="mt-6 space-y-4 text-base leading-7 text-ink-soft">
              <p>A Social Security Administration notice informs you of a determination about your benefits — retirement, disability (SSDI), SSI, Medicare, or survivor benefits. It may announce an overpayment, a cessation of benefits, a change in payment amount, or a request for updated information.</p>
              <p>SSA notices include appeal rights and deadlines. You typically have 60 days to request a reconsideration, and additional rights to request a hearing before an Administrative Law Judge. Missing these deadlines can result in benefit termination or overpayment collection.</p>
              <p>A documented response can request reconsideration, provide additional medical evidence, explain financial hardship for overpayment waiver, or correct information. Responding promptly preserves your appeal rights and can prevent benefit interruption.</p>
            </div>
            <div className="mt-8 rounded-lg border border-rule/60 bg-paper-deep/30 p-5">
              ...
              <ul className="mt-3 grid gap-2 sm:grid-cols-2">
                <li className="flex items-center gap-2 text-sm text-ink-soft"><span className="text-stamp">▸</span>SSA notice type and number</li>
                <li className="flex items-center gap-2 text-sm text-ink-soft"><span className="text-stamp">▸</span>Determination or decision</li>
                <li className="flex items-center gap-2 text-sm text-ink-soft"><span className="text-stamp">▸</span>Benefit type affected</li>
                <li className="flex items-center gap-2 text-sm text-ink-soft"><span className="text-stamp">▸</span>Amount (if overpayment or change)</li>
                <li className="flex items-center gap-2 text-sm text-ink-soft"><span className="text-stamp">▸</span>Appeal deadline (usually 60 days)</li>
                <li className="flex items-center gap-2 text-sm text-ink-soft"><span className="text-stamp">▸</span>Appeal level available</li>
                <li className="flex items-center gap-2 text-sm text-ink-soft"><span className="text-stamp">▸</span>SSA office address</li>
                <li className="flex items-center gap-2 text-sm text-ink-soft"><span className="text-stamp">▸</span>Required action or documentation</li>
              </ul>
            </div>
          </div>
        </section>
        ...
        <section className="border-b border-rule/60 bg-paper-deep/20">
          <div className="mx-auto max-w-4xl px-4 py-12 sm:px-6 sm:py-16">
            ...
            <div className="mt-8 grid gap-6 md:grid-cols-3">
              <ProcessStep number="01" title="Upload & analyze" text="Upload the SSA notice. AI extracts the determination, deadline, appeal rights, and amount — and identifies the appropriate appeal level." />
              <ProcessStep number="02" title="Review & draft" text="Add your evidence, medical records, or financial documentation. Generate a reconsideration request, waiver request, or hearing appeal." />
              <ProcessStep number="03" title="Mail with proof" text="Approve the exact draft. Certified mail provides proof of timely appeal — essential for preserving your 60-day appeal window." />
            </div>
          </div>
        </section>
        ...
        <section ref={workflowRef} className="border-b border-rule/60" style={{ scrollMarginTop: "80px" }}>
          <div className="mx-auto max-w-5xl px-4 py-12 sm:px-6">
            {workflowStarted ? (
              <WorkflowShell title="Respond to an SSA Notice" steps={STEPS} step={step} setStep={setStep} canContinue={canContinue()} onNext={next} onBack={() => setStep((s) => Math.max(s - 1, 0))}>
      {step === 0 && (
        <div>
          <div className="postmark w-fit">1 · Start</div>
          <h2 className="mt-4 font-serif text-4xl">Respond to an SSA Notice</h2>
          <p className="mt-3 text-muted-foreground">Organize a Social Security notice, its deadline, stated decision or request, and the records you need for a written response or next review step.</p>
          <div className="mt-6 rounded-md border border-rule/70 bg-paper-deep/40 p-4 text-sm text-muted-foreground">
            ...
            <p className="mt-2">{definition?.disclaimer ?? "SSA procedures have strict deadlines. Notice Respond is not a law firm and does not provide legal advice."}</p>
          </div>
          {(() => { const seo = getWorkflowSEO("ssa-notice"); return seo ? <FAQSection faq={seo.faq} /> : null; })()}
        </div>
      )}
      {step === 1 && (
        <div>
          ...
          <div className="mt-6 grid gap-4 sm:grid-cols-2">
            <div><label className="input-label">Agency</label><input className="input-field" value={agency} onChange={(e) => setAgency(e.target.value)} placeholder="Social Security Administration" /></div>
      ...
      {step === 4 && (
        <div>
          ...
          {llmAnalysis.llmAnalysis && (
            <button
              onClick={async () => {
                const r = await authFetch("/api/workflows/draft", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ workflowId: "ssa-notice", analysis: llmAnalysis.llmAnalysis, userFacts: facts, userObjective: objective }) });
      ...
      {step === 7 && (<div><div className="postmark w-fit">8 · Recipient</div><h2 className="mt-4 font-serif text-3xl">Where should we send it?</h2><p className="mt-3 text-muted-foreground">Enter the agency's mailing address.</p><RecipientForm recipient={recipient} setRecipient={setRecipient} orgPlaceholder={agency || "Social Security Administration"} /></div>)}
        ...
        <section className="border-b border-rule/60">
          <div className="mx-auto max-w-3xl px-4 py-12 sm:px-6">
            ...
            <div className="mt-6 grid gap-4 sm:grid-cols-3">
              <RelatedCard href="/workflows/benefits-notice" title="Benefits Notice" desc="Respond to other benefits agency notices" />
              <RelatedCard href="/workflows/uscis-notice" title="USCIS Notice" desc="Respond to immigration notices" />
