...
export const Route = createFileRoute("/workflows/benefits-notice")({
  head: () => createWorkflowHead("benefits-notice"),
  component: benefitsnotice,
});
...
function benefitsnotice() {
  const definition = workflows["benefits-notice"];
  ...
  const llmAnalysis = useCombinedAnalysis("benefits-notice");

  function generateDraft() {
    return `Re: Response to Respond to a Benefits Notice
...
[Your Name]`;
      ...
      try {
        if (llmAnalysis.llmAnalysis) {
          const r = await authFetch("/api/workflows/draft", {
            ...
            body: JSON.stringify({ workflowId: "benefits-notice", analysis: llmAnalysis.llmAnalysis, userFacts: facts, userObjective: objective }),
          });
          if (r.ok) {
  ...
  if (done) return <Success title="Your response has been submitted" href="/workflows/benefits-notice" />;
  ...
  return (
    <div className="min-h-screen bg-paper">
      ...
      <main>
        ...
        <section className="relative overflow-hidden border-b border-rule/60">
          ...
          <div className="relative mx-auto max-w-4xl px-4 py-14 sm:px-6 sm:py-20 md:py-28">
            <nav className="flex items-center gap-1.5 text-xs text-muted-foreground" aria-label="Breadcrumb">
              ...
              <span className="text-rule">/</span>
              <span className="text-ink-soft">Benefits Agency</span>
            </nav>
            <div className="postmark w-fit mt-6">Benefits Agency</div>
            <h1 className="mt-6 font-serif text-4xl leading-[1.1] sm:text-5xl md:text-6xl">
              Respond to your <span className="italic text-stamp">benefits notice</span>
            </h1>
            <p className="mt-6 max-w-2xl text-base leading-7 text-ink-soft sm:text-lg">
              A government benefits agency sent a notice about your eligibility, overpayment, or determination. Upload it, understand the decision, and prepare a documented response.
            </p>
            ...
            <div className="mt-12 grid grid-cols-2 gap-px overflow-hidden rounded-lg border border-rule/60 bg-rule/60 sm:grid-cols-4">
              <KeyFact label="Notice type" value="Benefits" />
        ...
        <section className="border-b border-rule/60">
          <div className="mx-auto max-w-3xl px-4 py-12 sm:px-6 sm:py-16">
            ...
            <h2 className="mt-3 font-serif text-3xl leading-tight">What is a benefits notice?</h2>
            <div className="mt-6 space-y-4 text-base leading-7 text-ink-soft">
              <p>A benefits notice is an official communication from a government agency that administers public assistance — unemployment insurance, SNAP/food assistance, Medicaid, housing benefits, VA benefits, or workers' compensation. It may announce an overpayment, eligibility determination, benefit reduction, or termination.</p>
              <p>These notices include appeal rights and deadlines, typically 30 to 60 days. Missing the deadline can result in automatic collection of overpayments, benefit termination, or denial of future benefits. Most agencies offer an administrative appeal process.</p>
              <p>A documented response can request a fair hearing, present evidence of eligibility, explain overpayment circumstances, or request a waiver. Responding promptly preserves your appeal rights and can prevent benefit interruption or collection action.</p>
            </div>
            <div className="mt-8 rounded-lg border border-rule/60 bg-paper-deep/30 p-5">
              ...
              <ul className="mt-3 grid gap-2 sm:grid-cols-2">
                <li className="flex items-center gap-2 text-sm text-ink-soft"><span className="text-stamp">▸</span>Agency name and case number</li>
                <li className="flex items-center gap-2 text-sm text-ink-soft"><span className="text-stamp">▸</span>Notice type (overpayment, termination, etc.)</li>
                <li className="flex items-center gap-2 text-sm text-ink-soft"><span className="text-stamp">▸</span>Benefit program affected</li>
                <li className="flex items-center gap-2 text-sm text-ink-soft"><span className="text-stamp">▸</span>Amount (if overpayment or adjustment)</li>
                <li className="flex items-center gap-2 text-sm text-ink-soft"><span className="text-stamp">▸</span>Appeal deadline</li>
                <li className="flex items-center gap-2 text-sm text-ink-soft"><span className="text-stamp">▸</span>Appeal or hearing rights</li>
                <li className="flex items-center gap-2 text-sm text-ink-soft"><span className="text-stamp">▸</span>Agency mailing address</li>
        ...
        <section className="border-b border-rule/60 bg-paper-deep/20">
          <div className="mx-auto max-w-4xl px-4 py-12 sm:px-6 sm:py-16">
            ...
            <div className="mt-8 grid gap-6 md:grid-cols-3">
              <ProcessStep number="01" title="Upload & analyze" text="Upload the benefits notice. AI extracts the agency, determination, deadline, and appeal rights — and identifies what needs your attention." />
              <ProcessStep number="02" title="Review & draft" text="Add your evidence, financial documentation, or explanation. Generate a fair hearing request, waiver application, or appeal letter." />
              <ProcessStep number="03" title="Mail with proof" text="Approve the exact draft. Certified mail provides proof of timely appeal — essential for preserving your hearing rights before the deadline." />
            </div>
          </div>
        </section>
        ...
        <section ref={workflowRef} className="border-b border-rule/60" style={{ scrollMarginTop: "80px" }}>
          <div className="mx-auto max-w-5xl px-4 py-12 sm:px-6">
            {workflowStarted ? (
              <WorkflowShell title="Respond to a Benefits Notice" steps={STEPS} step={step} setStep={setStep} canContinue={canContinue()} onNext={next} onBack={() => setStep((s) => Math.max(s - 1, 0))}>
      {step === 0 && (
        <div>
          <div className="postmark w-fit">1 · Start</div>
          <h2 className="mt-4 font-serif text-4xl">Respond to a Benefits Notice</h2>
          <p className="mt-3 text-muted-foreground">Understand a benefits notice, preserve the stated deadline, and prepare a factual response or request for review from your records.</p>
          <div className="mt-6 rounded-md border border-rule/70 bg-paper-deep/40 p-4 text-sm text-muted-foreground">
            ...
            <p className="mt-2">{definition?.disclaimer ?? "Benefits programs have specific rules and deadlines. Notice Respond is not a law firm and does not provide legal advice."}</p>
          </div>
          {(() => { const seo = getWorkflowSEO("benefits-notice"); return seo ? <FAQSection faq={seo.faq} /> : null; })()}
        </div>
      )}
      {step === 1 && (
        <div>
          ...
          <div className="mt-6 grid gap-4 sm:grid-cols-2">
            <div><label className="input-label">Agency</label><input className="input-field" value={agency} onChange={(e) => setAgency(e.target.value)} placeholder="Benefits Agency" /></div>
      ...
      {step === 4 && (
        <div>
          ...
          {llmAnalysis.llmAnalysis && (
            <button
              onClick={async () => {
                const r = await authFetch("/api/workflows/draft", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ workflowId: "benefits-notice", analysis: llmAnalysis.llmAnalysis, userFacts: facts, userObjective: objective }) });
      ...
      {step === 7 && (<div><div className="postmark w-fit">8 · Recipient</div><h2 className="mt-4 font-serif text-3xl">Where should we send it?</h2><p className="mt-3 text-muted-foreground">Enter the agency's mailing address.</p><RecipientForm recipient={recipient} setRecipient={setRecipient} orgPlaceholder={agency || "Benefits Agency"} /></div>)}
        ...
        <section className="border-b border-rule/60">
          <div className="mx-auto max-w-3xl px-4 py-12 sm:px-6">
            ...
            <div className="mt-6 grid gap-4 sm:grid-cols-3">
              <RelatedCard href="/workflows/ssa-notice" title="SSA Notice" desc="Respond to Social Security Administration notices" />
