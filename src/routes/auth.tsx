...
function AuthPage() {
  ...
  if (user) return <div className="min-h-screen"><SiteHeader /><main className="mx-auto max-w-3xl px-6 py-24 text-center"><div className="postmark mx-auto w-fit">MailMyPDF Account</div><h1 className="mt-6 font-serif text-4xl">You're already signed in.</h1><p className="mt-3 text-sm text-muted-foreground">Continue to your Notice Respond cases or manage your account.</p><div className="mt-8 flex justify-center gap-3"><Link to={(searchParams?.returnTo || "/dashboard") as "/dashboard"} className="rounded-full bg-primary px-6 py-3 text-sm font-medium text-primary-foreground">My cases</Link><Link to="/" className="rounded-full border border-input px-6 py-3 text-sm font-medium">Home</Link></div></main><SiteFooter /></div>;
